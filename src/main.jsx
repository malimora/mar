...
function rangeStartFromFilter(filter, now){ return getHistoryFilterStart(filter, now) || new Date(0); }
function statusPhrase(progress, dueNow, late, missingPain, btTrendUp){ if(progress>=0.95&&late===0) return "On track"; if(btTrendUp) return "Breakthrough use increasing"; if(missingPain) return "Pain data missing"; if(late>0||dueNow>0) return "Schedule drift detected"; return "Slightly behind"; }
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) {
  ...
  const todaysEvents=events.filter((e)=>{const t=new Date(e.actualAt).getTime(); return t>=dayStart.getTime()&&t<dayEnd.getTime();});
  const takenToday=todaysEvents.filter((e)=>e.status==="taken");
  const regularTakenToday=todaysEvents.filter((e)=>planMap[e.planId]?.kind==="required"&&e.status==="taken").length;
  const scheduleRows=regularPlans.flatMap((plan)=>(plan.baseTimes||[]).map((time,idx)=>{const mins=parseClockTime(time); if(mins==null) return null; const scheduledAt=new Date(dayStart); scheduledAt.setMinutes(mins,0,0); const actual=todaysEvents.filter((e)=>e.planId===plan.id&&e.status==="taken").sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt))[idx]||null; if(actual){ const dev=Math.round((new Date(actual.actualAt)-scheduledAt)/60000); return {id:`${plan.id}-${idx}`,plan,time,scheduledAt,actual,state:Math.abs(dev)<=15?"taken-on-time":"late",dev}; }
    const delta=(effectiveNow.getTime()-scheduledAt.getTime())/60000;
    const state=delta<0?"upcoming":delta<=graceMinutes?"due":"missed";
    return {id:`${plan.id}-${idx}`,plan,time,scheduledAt,actual:null,state,dev:null};
  })).filter(Boolean).sort((a,b)=>a.scheduledAt-b.scheduledAt);

  const scheduledRegular=scheduleRows.length;
  const progress=scheduledRegular?regularTakenToday/scheduledRegular:0;
  const dueNow=scheduleRows.filter((r)=>r.state==="due").length;
  const lateCount=scheduleRows.filter((r)=>r.state==="late").length;
  const missingPain=takenToday.filter((e)=>e.painBefore==null).length>0;
  const lastDose=[...takenToday].sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt))[0]||null;
  const nextRow=scheduleRows.find((r)=>r.state==="upcoming")||null;

  const rangeStart=rangeStartFromFilter(filter,effectiveNow);
  const inRangeEvents=events.filter((e)=>new Date(e.actualAt)>=rangeStart).sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt));
  const painSeries=inRangeEvents.filter((e)=>e.painBefore!=null&&Number.isFinite(Number(e.painBefore))).map((e)=>({t:new Date(e.actualAt),pain:Number(e.painBefore),planId:e.planId}));

  const bt3=Array.from({length:3}).map((_,i)=>{const d=startOfDay(effectiveNow); d.setDate(d.getDate()-(2-i)); const e=new Date(d); e.setDate(e.getDate()+1); const list=events.filter((x)=>x.planId==="prn"&&x.status==="taken"&&new Date(x.actualAt)>=d&&new Date(x.actualAt)<e); return {label:formatDate(d),count:list.length,list};});
  const btTrendUp=(bt3[2].count>bt3[1].count)&&(bt3[1].count>=bt3[0].count);
  const btPainAll=bt3.flatMap((d)=>d.list).map((e)=>Number(e.painBefore)).filter((n)=>Number.isFinite(n));
  const btAvg=btPainAll.length?(btPainAll.reduce((a,b)=>a+b,0)/btPainAll.length):null;

  const paracetamolTaken=todaysEvents.filter((e)=>e.status==="taken"&&Number(planMap[e.planId]?.paracetamolMg)>0).sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt));
  let shortestGap=null; for(let i=1;i<paracetamolTaken.length;i+=1){const gap=(new Date(paracetamolTaken[i].actualAt)-new Date(paracetamolTaken[i-1].actualAt))/60000; if(shortestGap===null||gap<shortestGap) shortestGap=gap;}
  const timingWarning=shortestGap!=null&&shortestGap<240;

  const doseImpacts=takenToday.filter((e)=>e.status==="taken").map((dose)=>{const before=Number.isFinite(Number(dose.painBefore))?Number(dose.painBefore):null; const after=Number.isFinite(Number(dose.painAfter))?Number(dose.painAfter):null; if(before!=null&&after!=null) return {id:dose.id,label:planMap[dose.planId]?.label||dose.planId,before,after,estimated:false}; if(before==null) return {id:dose.id,label:planMap[dose.planId]?.label||dose.planId,before:null,after:null,estimated:false}; const doseTime=new Date(dose.actualAt).getTime(); const futurePain=painSeries.find((p)=>p.t.getTime()>doseTime+30*60000&&p.t.getTime()<=doseTime+4*3600000); return {id:dose.id,label:planMap[dose.planId]?.label||dose.planId,before,after:futurePain?futurePain.pain:null,estimated:Boolean(futurePain)}; }).slice(-4);

  const phrase=statusPhrase(progress,dueNow,lateCount,missingPain,btTrendUp);

  return <div className="space-y-4"><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-slate-900">Today</h2><div className="text-sm font-medium text-slate-500">{Math.round(progress*100)}%</div></div><div className="mt-2 text-sm font-semibold text-slate-800">{phrase}</div><div className="mt-3 flex items-center gap-3"><div className="h-14 w-14 rounded-full border-4 border-slate-200 grid place-items-center text-xs font-semibold text-slate-700">{regularTakenToday}/{scheduledRegular}</div><div className="text-sm text-slate-700"><div>Next: {nextRow?`${medCode(nextRow.plan.id)} ${nextRow.plan.medication} ${formatTime(nextRow.scheduledAt)}`:"No more regular doses"}</div><div>Last: {lastDose?`${medCode(lastDose.planId)} ${formatTime(lastDose.actualAt)}`:"Not logged"}</div></div></div>{timingWarning?<div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">! Timing check: paracetamol gaps below 4h today.</div>:null}</section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Medication map</h2><div className="mt-2 text-xs text-slate-500">● taken  ○ planned  × missed  ▲ breakthrough</div><div className="mt-3 space-y-2">{scheduleRows.map((r)=><div key={r.id} className="flex items-center justify-between text-sm"><div className="text-slate-600">{formatTime(r.scheduledAt)} {medCode(r.plan.id)}</div><div className={classNames("font-medium",r.state==="taken-on-time"?"text-emerald-700":r.state==="late"?"text-amber-700":r.state==="missed"?"text-rose-700":"text-slate-500")}>{r.state==="taken-on-time"?"● taken":r.state==="late"?`● ${formatSignedMinutes(r.dev)} late`:r.state==="due"?"○ due now":r.state==="upcoming"?"○ upcoming":"× missed"}</div></div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Timing</h2><div className="mt-3 space-y-2">{scheduleRows.map((r)=><div key={`${r.id}-drift`} className="text-sm"><div className="flex items-center justify-between"><span className="text-slate-700">{formatTime(r.scheduledAt)} {medCode(r.plan.id)}</span><span className="text-slate-600">{r.state==="late"?`${formatSignedMinutes(r.dev)} late`:r.state==="taken-on-time"?"on time":r.state}</span></div>{r.state==="late"?<div className="mt-1 h-1.5 rounded-full bg-amber-100"><div className="h-1.5 rounded-full bg-amber-500" style={{width:`${Math.min(100,Math.abs(r.dev))}%`}} /></div>:null}</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Pain trend</h2><div className="mt-4 flex flex-wrap gap-2">{[["today","Today"],["3d","3 days"],["7d","7 days"]].map(([key,label])=><button key={key} onClick={()=>onFilterChange(key)} className={classNames("rounded-full px-4 py-2 text-sm font-medium",filter===key?"bg-slate-900 text-white":"bg-slate-100 text-slate-700")}>{label}</button>)}</div><div className="mt-3 space-y-1">{painSeries.length===0?<div className="text-sm text-slate-500">No pain scores in this range.</div>:painSeries.slice(-10).map((p)=><div key={`${p.t.toISOString()}-${p.planId}`} className="text-sm text-slate-700">{formatDateTime(p.t)} · {p.pain}/10 · {medCode(p.planId)}</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Dose impact</h2><div className="mt-3 space-y-2">{doseImpacts.length===0?<div className="text-sm text-slate-500">Not enough data.</div>:doseImpacts.map((d)=><div key={d.id} className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">{d.label}: {d.before==null?"not enough data":d.after==null?`${d.before} → —`:`${d.before} → ${d.after} ${d.after<=d.before?`↓ ${d.before-d.after}`:`↑ ${d.after-d.before}`}`}{d.estimated?" (estimated)":""}</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Breakthrough</h2><div className="mt-2 space-y-1">{bt3.map((d)=><div key={d.label} className="text-sm text-slate-700">{d.label} {"█".repeat(d.count)||"—"}</div>)}</div><div className="mt-2 text-sm text-slate-700">Pain before breakthrough: {btAvg==null?"Not enough data":`${btAvg.toFixed(1)} / 10 avg`}</div></section>

  <details className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" open={false}><summary className="cursor-pointer text-lg font-semibold text-slate-900">Log</summary><div className="mt-3 space-y-2">{inRangeEvents.length===0?<div className="text-sm text-slate-500">No events in this range.</div>:inRangeEvents.slice().reverse().map((e)=><div key={e.id} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2 text-sm"><div>{formatTime(e.actualAt)} {medCode(e.planId)} {(planMap[e.planId]?.medication)||e.planId}</div><button onClick={()=>onDelete(e.id)} className="rounded-xl border border-slate-200 px-2 py-1 text-xs text-slate-600">Delete</button></div>)}</div></details></div>;
}
