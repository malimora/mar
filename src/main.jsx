...
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) { const careStart=getCareDayStart(effectiveNow); const careEnd=getCareDayEnd(effectiveNow); const slots=generatePlannedSlots(plans, careStart); const {matched,extras}=matchActualEventsToPlannedSlots(events.filter((e)=>new Date(e.actualAt)>=careStart&&new Date(e.actualAt)<careEnd), slots);
