...
const controlClassName = "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-slate-300 focus:ring-2 focus:ring-slate-200";
...
function Field({ label, children }) { return <label className="block min-w-0"><div className="mb-2 text-sm font-medium text-slate-700">{label}</div>{children}</label>; }
...
return <div className="grid gap-4 xl:grid-cols-[1.2fr,0.8fr]"><section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-center gap-2"><SettingsIcon className="h-5 w-5 text-slate-700" /><h2 className="text-lg font-semibold text-slate-900">Plan settings</h2></div><div className="mt-4 space-y-4">{draftPlans.map((plan)=><div key={plan.id} className="rounded-3xl border border-slate-200 p-4"><div className="mb-4 text-base font-semibold text-slate-900">{plan.label}</div><div className="grid gap-4 sm:grid-cols-2"><Field label="Medication name"><input value={plan.medication} onChange={(e)=>updatePlan(plan.id,{medication:e.target.value})} className={controlClassName}/></Field><Field label="Interval (minutes)"><input type="number" min={1} step={1} value={plan.intervalMinutes} onChange={(e)=>updatePlan(plan.id,{intervalMinutes:e.target.value})} className={controlClassName}/></Field><Field label="Reference times"><input value={Array.isArray(plan.baseTimes)?plan.baseTimes.join(", "):String(plan.baseTimes||"")} onChange={(e)=>updatePlan(plan.id,{baseTimes:e.target.value})} className={controlClassName} placeholder="06:00, 12:00, 18:00"/></Field><Field label="Paracetamol per taken dose (mg)"><input type="number" min={0} step={1} value={plan.paracetamolMg} onChange={(e)=>updatePlan(plan.id,{paracetamolMg:e.target.value})} className={controlClassName}/></Field></div></div>)}<Field label="Shared Tramadol spacing (minutes)"><input type="number" min={0} step={1} value={settings.tramadolSpacingMinutes} onChange={(e)=>onChange({...settings,tramadolSpacingMinutes:Number.isFinite(Number(e.target.value))&&Number(e.target.value)>=0?Number(e.target.value):0})} className={controlClassName}/></Field><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><button onClick={savePlans} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white">Save plan changes</button><button onClick={onReset} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Reset defaults</button></div></div></section><section className="space-y-4"><div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-center gap-2"><BellIcon className="h-5 w-5 text-slate-700" /><h2 className="text-lg font-semibold text-slate-900">Reminder preferences</h2></div><div className="mt-4 space-y-3">{[["regularDue","Regular dose due"],["prnCheckIn","As-needed check-in"],["missedRegular","Missed regular dose"],["midnightDose","Bedtime / midnight dose"]].map(([key,label])=><label key={key} className="flex items-center justify-between gap-4 rounded-2xl bg-slate-50 px-4 py-3"><span className="text-sm text-slate-700">{label}</span><input type="checkbox" checked={Boolean(settings.reminders[key])} onChange={(e)=>onChange({...settings,reminders:{...settings.reminders,[key]:e.target.checked}})} className="h-4 w-4"/></label>)}</div><div className="mt-4 rounded-2xl border border-slate-200 p-4 text-sm text-slate-600"><div>Browser notification permission: <span className="font-medium text-slate-900">{notificationPermission}</span></div><button onClick={onRequestNotifications} className="mt-3 rounded-2xl border border-slate-200 bg-white px-4 py-2 font-semibold text-slate-700">Enable notifications</button><p className="mt-3 leading-6 text-slate-500">Manual time entry happens when logging an event, so the main screen stays calmer and simpler.</p></div></div><div className="rounded-3xl border border-amber-200 bg-amber-50/70 p-4 shadow-sm sm:p-5"><h3 className="text-base font-semibold text-slate-900">Rolling dose logic</h3><p className="mt-2 text-sm leading-6 text-slate-700">Regular doses become due from the last regular dose time plus the regular interval. PRN remains optional, but respects its own interval and the shared Tramadol spacing rule.</p></div></section></div>; }
...
function ActionSheet({ open, plans, events, settings, effectiveNow, sheet, onClose, onConfirm }) {
  ...
  return <div className="fixed inset-0 z-30 flex items-end justify-center bg-slate-900/40 p-2 sm:p-3 sm:items-center"><div className="w-full max-w-lg rounded-3xl bg-white p-4 shadow-2xl sm:p-6 max-h-[92vh] overflow-y-auto"><div className="flex items-start justify-between gap-4"><div><h3 className="text-xl font-semibold text-slate-900">Log {plan.label.toLowerCase()} event</h3><p className="mt-1 text-sm text-slate-600">{plan.medication}</p></div><button onClick={onClose} className="rounded-2xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700">Close</button></div>{spacingWarning?<div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><div className="font-semibold">Tramadol timing warning</div><div className="mt-1">A Tramadol event was logged {formatRelative(spacingWarning.elapsed)} earlier at {formatDateTime(spacingWarning.previous.actualAt)}.</div></div>:null}<div className="mt-4 grid gap-4"><Field label="Status"><select value={status} onChange={(event) => setStatus(event.target.value)} className={controlClassName}>{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Actual date and time"><input type="datetime-local" value={actualAt} onChange={(event) => setActualAt(event.target.value)} className={controlClassName} /></Field>{status === "taken" || plan.kind === "optional" ? <Field label="Pain before (optional)"><input type="number" min={0} max={10} step={1} value={painBefore} onChange={(event) => setPainBefore(event.target.value)} className={controlClassName} placeholder="0 to 10" /></Field> : null}{status === "taken" ? <Field label="Pain after (optional)"><input type="number" min={0} max={10} step={1} value={painAfter} onChange={(event) => setPainAfter(event.target.value)} className={controlClassName} placeholder="0 to 10" /></Field> : null}<Field label={plan.kind === "optional" && status === "taken" ? "Why taking this? (optional)" : "Note (optional)"}><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={4} className={controlClassName} placeholder={plan.kind === "optional" && status === "taken" ? "Pain flare after physio, night pain, swelling..." : "Anything useful to remember"} /></Field><div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">The time defaults to now, but you can override it here for historical entries or corrections.</div></div><div className="mt-5 flex flex-col gap-3 sm:flex-row"><button onClick={() => { const parsedDate = fromLocalInputValue(actualAt); if (!parsedDate) { alert("Please enter a valid date and time."); return; } onConfirm({ planId: plan.id, status, actualAt: parsedDate.toISOString(), painBefore: parsePainValue(painBefore), painAfter: parsePainValue(painAfter), note: String(note || "").trim() }); }} className="flex-1 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Save event</button><button onClick={onClose} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700">Cancel</button></div></div></div>;
}
