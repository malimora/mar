...
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) { const planMap=getPlanMap(plans); const careStart=getCareDayStart(effectiveNow); const careEnd=getCareDayEnd(effectiveNow); const slots=generatePlannedSlots(plans, careStart); const {matched}=matchActualEventsToPlannedSlots(events.filter((e)=>new Date(e.actualAt)>=careStart&&new Date(e.actualAt)<careEnd), slots);
 ...
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm overflow-x-auto"><h2 className="text-lg font-semibold">Pain trend</h2><div className="mt-3 flex gap-2">{[['today','Today'],['3d','3 days'],['7d','7 days']].map(([k,l])=><button key={k} onClick={()=>onFilterChange(k)} className={classNames('rounded-full px-3 py-1 text-sm',filter===k?'bg-slate-900 text-white':'bg-slate-100')}>{l}</button>)}</div>{painPoints.length===0?<div className="mt-2 text-sm text-slate-500">No pain scores logged for this range.</div>:<svg viewBox="0 0 1000 180" className="mt-2 min-w-[700px] w-full h-44"><text x="8" y="20" fontSize="10">10</text><text x="8" y="92" fontSize="10">5</text><text x="8" y="164" fontSize="10">0</text><path d={painPath} fill="none" stroke="#0f172a" strokeWidth="2"/>{painPoints.map((p,i)=><circle key={i} cx={40+xFor(p.t)*9.2} cy={10+(100-(p.pain/10)*100)*1.5} r="3" fill="#0f172a"/> )}</svg>}</section>
