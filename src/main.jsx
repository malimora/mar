...
function OverviewCard({ regularStates, prnState, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  ...
  const primaryTitle = !leadRegular ? "No regular dose logged yet" : overdueRegular.length ? `${leadRegular.plan.label} overdue` : `Next regular: ${leadRegular.plan.label} at ${formatTime(leadRegular.state.nextDueAt)}`;
  const primarySubtitle = !leadRegular ? "Log the first regular dose to start the rolling schedule." : overdueRegular.length ? `${overdueDuration} late` : `In ${formatRelative(leadRegular.state.nextDueAt.getTime() - effectiveNow.getTime())}`;
  const dueLine = overdueRegular.length && leadRegular?.state.nextDueAt ? `Due since ${formatDateTime(leadRegular.state.nextDueAt)}` : null;
