...
function minutesSinceStartOfDay(dateLike){ const d=new Date(dateLike); return d.getHours()*60+d.getMinutes(); }
function formatSignedMinutes(totalMinutes){ if(totalMinutes==null||!Number.isFinite(totalMinutes)) return "—"; const sign=totalMinutes>0?"+":""; const abs=Math.abs(totalMinutes); const h=Math.floor(abs/60); const m=abs%60; if(h) return `${sign}${h}h ${pad(m)}m`; return `${sign}${m}m`; }
function driftLabel(minutes){ if(minutes==null||!Number.isFinite(minutes)) return "Missed"; const abs=Math.abs(minutes); if(abs<=15) return "On time"; if(abs<=45) return "A bit late"; return "Very late"; }
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) {
  const filtered=filterEventsByHistory(events,filter,effectiveNow);
  ...
  const scheduledRegular=regularPlans.reduce((sum,p)=>sum+((Array.isArray(p.baseTimes)?p.baseTimes.length:0)),0);
  const regularTakenToday=todaysEvents.filter((e)=>planMap[e.planId]?.kind==="required"&&e.status==="taken").length;
  const lastDose=[...todaysEvents].filter((e)=>e.status==="taken").sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt))[0]||null;
  const nextRegular=regularPlans.flatMap((plan)=>(plan.baseTimes||[]).map((time)=>({plan,time,minutes:parseClockTime(time)}))).filter((x)=>x.minutes!==null).map((x)=>{const due=new Date(dayStart); due.setMinutes(x.minutes,0,0); return {...x,due};}).filter((x)=>x.due.getTime()>effectiveNow.getTime()).sort((a,b)=>a.due-b.due)[0]||null;
  const currentPain=[...todaysEvents].filter((e)=>e.painBefore!=null&&Number.isFinite(Number(e.painBefore))).sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt))[0]?.painBefore;
  const driftRows=regularPlans.flatMap((plan)=> (plan.baseTimes||[]).map((time,idx)=>{ const scheduledMinutes=parseClockTime(time); if(scheduledMinutes===null) return null; const scheduledAt=new Date(dayStart); scheduledAt.setMinutes(scheduledMinutes,0,0); const takenSamePlan=todaysEvents.filter((e)=>e.planId===plan.id&&e.status==="taken").sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt)); const actual=takenSamePlan[idx]||null; const deviation=actual?Math.round((new Date(actual.actualAt)-scheduledAt)/60000):null; return {id:`${plan.id}-${time}-${idx}`,label:`${plan.label} ${time}`,scheduledAt,actual,deviation}; })).filter(Boolean);
  const painPoints=[...todaysEvents].filter((e)=>e.painBefore!=null&&Number.isFinite(Number(e.painBefore))).map((e)=>({time:new Date(e.actualAt),pain:Number(e.painBefore),planId:e.planId})).sort((a,b)=>a.time-b.time);
  const impactRows=[...todaysEvents].filter((e)=>e.status==="taken"&&e.painBefore!=null&&e.painAfter!=null&&Number.isFinite(Number(e.painBefore))&&Number.isFinite(Number(e.painAfter))).map((e)=>({id:e.id,label:planMap[e.planId]?.label||e.planId,before:Number(e.painBefore),after:Number(e.painAfter),delta:Number(e.painAfter)-Number(e.painBefore)}));
  const breakthroughToday=todaysEvents.filter((e)=>e.planId==="prn"&&e.status==="taken");
  const last3Days=Array.from({length:3}).map((_,i)=>{const d=startOfDay(effectiveNow); d.setDate(d.getDate()-(2-i)); const e=new Date(d); e.setDate(e.getDate()+1); const count=events.filter((x)=>x.planId==="prn"&&x.status==="taken"&&new Date(x.actualAt)>=d&&new Date(x.actualAt)<e).length; return {label:formatDate(d),count};});
  const avgPainBeforeBt=breakthroughToday.length? (breakthroughToday.reduce((sum,e)=>sum+(Number.isFinite(Number(e.painBefore))?Number(e.painBefore):0),0)/breakthroughToday.filter((e)=>Number.isFinite(Number(e.painBefore))).length):null;
  const paracetamolTakenToday=todaysEvents.filter((e)=>e.status==="taken"&&Number(planMap[e.planId]?.paracetamolMg)>0).sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt));
  let shortestGap=null; for(let i=1;i<paracetamolTakenToday.length;i+=1){const gap=(new Date(paracetamolTakenToday[i].actualAt)-new Date(paracetamolTakenToday[i-1].actualAt))/60000; if(shortestGap===null||gap<shortestGap) shortestGap=gap;}
  const timingWarning=shortestGap!==null&&shortestGap<240;

  return <div className="space-y-4"><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-slate-900">Today&apos;s plan</h2><div className="text-sm text-slate-500">Regular doses completed: {scheduledRegular?Math.round((regularTakenToday/scheduledRegular)*100):0}%</div></div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><SummaryTile label="Progress" value={`${regularTakenToday} of ${scheduledRegular} regular doses`} /><SummaryTile label="Next regular dose" value={nextRegular?`${nextRegular.plan.label} at ${formatTime(nextRegular.due)}`:"No remaining regular dose today"} /><SummaryTile label="Last dose" value={lastDose?`${planMap[lastDose.planId]?.medication||lastDose.planId} at ${formatTime(lastDose.actualAt)}`:"Not logged"} /><SummaryTile label="Current pain" value={currentPain!=null?`${currentPain}/10`:"Not logged"} /></div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Medication timeline (24h)</h2><div className="mt-3 space-y-2">{todaysEvents.length===0?<div className="text-sm text-slate-500">No doses logged today.</div>:todaysEvents.sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt)).map((e)=><div key={e.id} className="text-sm text-slate-700">{formatTime(e.actualAt)} · {planMap[e.planId]?.label||e.planId} · {formatStatus(e.status)}</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Schedule drift</h2><div className="mt-3 space-y-2">{driftRows.length===0?<div className="text-sm text-slate-500">No regular schedule configured.</div>:driftRows.map((row)=><div key={row.id} className="rounded-2xl bg-slate-50 p-3 text-sm"><div className="font-medium text-slate-900">{row.label}</div><div className="text-slate-700">Scheduled {formatTime(row.scheduledAt)} · {row.actual?`Actual ${formatTime(row.actual.actualAt)} (${formatSignedMinutes(row.deviation)})`:"Missed"}</div><div className="text-xs text-slate-500">{driftLabel(row.deviation)}</div></div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Pain over time</h2><div className="mt-3 space-y-2">{painPoints.length===0?<div className="text-sm text-slate-500">No pain scores logged today.</div>:painPoints.map((p)=><div key={`${p.planId}-${p.time.toISOString()}`} className="text-sm text-slate-700">{formatTime(p.time)} · Pain {p.pain}/10 · {planMap[p.planId]?.label||p.planId}</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Pain change after dose</h2><div className="mt-3 space-y-2">{impactRows.length===0?<div className="text-sm text-slate-500">No before/after pain pairs logged yet.</div>:impactRows.map((row)=><div key={row.id} className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">{row.label}: {row.before} → {row.after} ({row.delta<=0?`↓ ${Math.abs(row.delta)}`:`↑ ${row.delta}`})</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Breakthrough use</h2><div className="mt-3 grid gap-3 sm:grid-cols-2"><SummaryTile label="Today" value={String(breakthroughToday.length)} /><SummaryTile label="Pain before breakthrough" value={avgPainBeforeBt!=null&&Number.isFinite(avgPainBeforeBt)?`${avgPainBeforeBt.toFixed(1)}/10`:"Not enough data"} /></div><div className="mt-3 space-y-1">{last3Days.map((d)=><div key={d.label} className="text-sm text-slate-700">{d.label}: {"█".repeat(d.count)||"—"} ({d.count})</div>)}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Timing checks</h2><div className="mt-3 text-sm text-slate-700">Paracetamol doses in last 24h: {paracetamolTakenToday.length}</div><div className="text-sm text-slate-700">Shortest gap: {shortestGap==null?"—":formatSignedMinutes(Math.round(shortestGap)).replace("+","")}</div><div className={classNames("mt-2 rounded-2xl p-3 text-sm",timingWarning?"bg-amber-50 text-amber-900 border border-amber-200":"bg-emerald-50 text-emerald-900 border border-emerald-200")}>{timingWarning?"Paracetamol doses were less than 4 hours apart. Check your medication instructions before taking more.":"No timing warnings."}</div></section>

  <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Event history</h2><div className="mt-4 flex flex-wrap gap-2">{[["today","Today"],["3d","3 days"],["7d","7 days"],["all","All"]].map(([key,label])=><button key={key} onClick={()=>onFilterChange(key)} className={classNames("rounded-full px-4 py-2 text-sm font-medium transition",filter===key?"bg-slate-900 text-white":"bg-slate-100 text-slate-700 hover:bg-slate-200")}>{label}</button>)}</div><div className="mt-4 space-y-6">{filtered.length===0?<div className="rounded-2xl border border-dashed border-slate-200 p-5 text-sm text-slate-500">No events in this range.</div>:Object.keys(grouped).sort((a,b)=>new Date(b)-new Date(a)).map((key)=><div key={key}><div className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">{formatDate(key)}</div><div className="space-y-3">{grouped[key].sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt)).map((event)=><div key={event.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-sm font-semibold text-slate-900">{(planMap[event.planId]&&planMap[event.planId].label)||event.planId} · {formatStatus(event.status)}</div><div className="mt-1 text-sm text-slate-600">{(planMap[event.planId]&&planMap[event.planId].medication)||"Unknown medication"}</div><div className="mt-1 text-xs text-slate-500">{formatDateTime(event.actualAt)}</div></div><div className="flex items-center gap-3"><div className="text-right text-sm text-slate-600">{event.painBefore != null && Number.isFinite(Number(event.painBefore))?<div>Pain before: {event.painBefore}/10</div>:null}{event.painAfter != null && Number.isFinite(Number(event.painAfter))?<div>Pain after: {event.painAfter}/10</div>:null}</div><button onClick={() => onDelete(event.id)} className="rounded-2xl border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-50"><TrashIcon className="h-4 w-4" /></button></div></div>{event.note?<div className="mt-3 whitespace-pre-wrap text-sm text-slate-600">{event.note}</div>:null}</div>)}</div></div>)}</div></section></div>;
}
