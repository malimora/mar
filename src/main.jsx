...
const DEFAULT_PLANS = [
  ...
  { id: "prn", label: "As needed Tramadol", medication: "Tramadol", intervalMinutes: 240, baseTimes: ["10:00", "16:00", "22:00"], containsTramadol: true, kind: "optional", paracetamolMg: 0 },
];
...
function getPrnPlanState(plan, events, settings, now) { if(!plan) return {lastTaken:null,availableAt:now,available:false}; const lastTaken=getLatestTakenEventByPlan(events, plan.id, now); const lastTramadol=getLatestTramadolEvent(events, now); const nextByPlan=lastTaken?new Date(new Date(lastTaken.actualAt).getTime()+plan.intervalMinutes*60000):null; const nextBySafety=lastTramadol?new Date(new Date(lastTramadol.actualAt).getTime()+settings.tramadolSpacingMinutes*60000):null; const c=[nextByPlan,nextBySafety].filter(Boolean); const availableAt=c.length?new Date(Math.max(...c.map((d)=>d.getTime()))):now; return {lastTaken,availableAt,available:now.getTime()>=availableAt.getTime()}; }
function getMostOverdueRequiredPlanState(plans, events, now) {
  const requiredPlans = (Array.isArray(plans) ? plans : []).filter((plan) => plan.kind === "required");
  const states = requiredPlans.map((plan) => ({ plan, state: getRegularPlanState(plan, events, now) }));
  const overdue = states.filter(({ state }) => state.status === "overdue" && state.nextDueAt);
  if (overdue.length) {
    return overdue.sort((a, b) => a.state.nextDueAt.getTime() - b.state.nextDueAt.getTime())[0];
  }
  const tracking = states.filter(({ state }) => state.nextDueAt);
  if (!tracking.length) return null;
  return tracking.sort((a, b) => a.state.nextDueAt.getTime() - b.state.nextDueAt.getTime())[0];
}
...
function OverviewCard({ primaryRegular, prnState, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  const regularState = primaryRegular?.state || { status: "empty", nextDueAt: null };
  const regularPlan = primaryRegular?.plan || null;
  ...
  const overdueDuration = regularState.nextDueAt ? formatRelative(effectiveNow.getTime() - regularState.nextDueAt.getTime()) : null;
  const primaryTitle = regularState.status === "empty" ? "No regular dose logged yet" : regularState.status === "overdue" ? `${regularPlan?.label || "Regular dose"} overdue` : `Next due: ${regularPlan?.label || "Regular dose"} at ${formatTime(regularState.nextDueAt)}`;
  const primarySubtitle = regularState.status === "empty" ? "Log the first regular dose to start the rolling schedule." : regularState.status === "overdue" ? `${overdueDuration} late` : `In ${formatRelative(regularState.nextDueAt.getTime() - effectiveNow.getTime())}`;
  const dueLine = regularState.status === "overdue" && regularState.nextDueAt ? `Due since ${formatDateTime(regularState.nextDueAt)}` : null;
  ...
  return <section className={classNames("rounded-3xl border p-5 shadow-sm sm:p-6", tone)}><div className="flex items-start justify-between gap-3"><h2 className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{primaryTitle}</h2><div className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200"><ClockIcon className="h-3.5 w-3.5" />Updated {formatTime(effectiveNow)}</div></div>{regularPlan?.medication?<p className="mt-1 text-sm text-slate-600">Plan: {regularPlan.medication}</p>:null}<p className="mt-2 text-xl font-semibold text-slate-900">{primarySubtitle}</p>{dueLine?<p className="mt-1 text-base text-slate-700">{dueLine}</p>:null}<div className="mt-3 space-y-1 text-sm leading-6 text-slate-700"><div>{prnLine}</div><div>{safetyLine}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:max-w-xl"><button onClick={() => regularPlan && onOpen(regularPlan.id, "taken")} className="min-h-12 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">Log due regular dose</button><button onClick={() => onOpen("prn", "taken")} className="min-h-12 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Log breakthrough dose</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3"><OverviewMetric label="Most recent tramadol" value={lastTramadol ? formatDateTime(lastTramadol.actualAt) : "Not logged"} /><OverviewMetric label="Breakthrough dose" value={prnState.available ? "Available now" : `In ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}`} /><OverviewMetric label="Paracetamol since midnight" value={`${paracetamolToday} mg`} /></div></section>;
}
