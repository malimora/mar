...
function TodayDashboard({ events, plans, prnState, effectiveNow, onOpen, onDelete }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-semibold">Regular progress</h3>{regularPlans.map((plan)=>{const taken=slots.filter((s)=>s.tracks.find((t)=>t.plan.id===plan.id&&t.matched)).length; return <div key={plan.id} className="mt-2"><p className="text-sm">{plan.medication.includes('Tramadol')?'Tramadol':'Paracetamol'} · {taken} / 4 taken</p><div className="mt-1 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-500" style={{width:`${(taken/4)*100}%`}} /></div></div>})}</section>
    </div>
