...
function OverviewCard({ regularState, prnState, lastRegular, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  ...
  const regularPlan = (settings.plans || []).find((plan) => plan.id === "regular");
  ...
  const primaryTitle = regularState.status === "empty" ? "No regular dose logged yet" : regularState.status === "overdue" ? "Regular schedule overdue" : `Next regular at ${formatTime(regularState.nextDueAt)}`;
