...
function AuthScreen({ mode, email, password, loading, errorMessage, onModeChange, onEmailChange, onPasswordChange, onSubmit }) {
  const actionLabel = mode === "sign-in" ? "Sign in" : "Create account";
  return <div className="min-h-screen bg-slate-50 px-4 py-10 text-slate-900 sm:px-6"><div className="mx-auto w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Medication Schedule</p><h1 className="mt-2 text-2xl font-semibold text-slate-900">Sign in</h1><p className="mt-2 text-sm text-slate-600">Use your Supabase Auth email and password to access the app.</p><form className="mt-6 space-y-4" onSubmit={onSubmit}><Field label="Email"><input type="email" value={email} onChange={(event) => onEmailChange(event.target.value)} autoComplete="email" required className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-slate-300" /></Field><Field label="Password"><input type="password" value={password} onChange={(event) => onPasswordChange(event.target.value)} autoComplete={mode === "sign-in" ? "current-password" : "new-password"} required minLength={6} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-slate-300" /></Field>{errorMessage ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-800">{errorMessage}</div> : null}<button type="submit" disabled={loading} className="w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Working..." : actionLabel}</button></form><button onClick={() => onModeChange(mode === "sign-in" ? "sign-up" : "sign-in")} className="mt-4 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700">{mode === "sign-in" ? "Need an account? Create one" : "Already have an account? Sign in"}</button></div></div>;
}

function MedicationSchedulePWA() {
  const [mounted, setMounted] = useState(false); const [tab, setTab] = useState("today"); const [session, setSession] = useState(null); const [authMode, setAuthMode] = useState("sign-in"); const [authEmail, setAuthEmail] = useState(""); const [authPassword, setAuthPassword] = useState(""); const [authLoading, setAuthLoading] = useState(false); const [authError, setAuthError] = useState(""); const [liveNow, setLiveNow] = useState(new Date()); const [data, setData] = useState(getInitialState()); const [installPromptEvent, setInstallPromptEvent] = useState(null); const [notificationPermission, setNotificationPermission] = useState(typeof Notification !== "undefined" ? Notification.permission : "unsupported"); const [sheet, setSheet] = useState({ open: false, planId: null, status: "taken" });
  ...
  useEffect(() => { if (!supabase) return; let active = true; supabase.auth.getSession().then(({ data: authData, error }) => { if (!active) return; if (error) { logSupabaseError("auth:getSession", error); return; } setSession(authData.session || null); }); const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => { setSession(nextSession || null); }); return () => { active = false; listener.subscription.unsubscribe(); }; }, []);

  useEffect(() => { if (!mounted || !supabase || !session) return; let cancelled = false; (async () => { try { logSupabase("sync:read:attempt"); const { data: rows, error } = await supabase.from("medication_events").select("*").order("actual_at", { ascending: true }); if (error) throw error; if (cancelled) return; logSupabase("sync:read:success", { rowCount: Array.isArray(rows) ? rows.length : 0 }); const remoteEvents = (rows || []).map(mapDbEventToApp); setData((current) => normalizeAppState({ ...current, events: mergeEventsPreferNewest(current.events, remoteEvents, current.settings.plans) })); } catch (error) { logSupabaseError("sync:read", error); } })(); return () => { cancelled = true; logSupabase("sync:read:cancelled"); }; }, [mounted, session]);
  useEffect(() => { if (!mounted || !supabase || !session) return; (async () => { try { const rows = data.events.map((event) => mapAppEventToDb(event)); if (!rows.length) return; logSupabase("sync:backfill:attempt", { rowCount: rows.length }); const { error } = await supabase.from("medication_events").upsert(rows, { onConflict: "id" }); if (error) throw error; logSupabase("sync:backfill:success", { rowCount: rows.length }); } catch (error) { logSupabaseError("sync:backfill", error, { rowCount: data.events.length }); } })(); }, [mounted, session, data.events]);
  ...
  async function handleAuthSubmit(event) { event.preventDefault(); if (!supabase) { setAuthError("Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY."); return; } setAuthLoading(true); setAuthError(""); try { if (authMode === "sign-in") { const { error } = await supabase.auth.signInWithPassword({ email: authEmail.trim(), password: authPassword }); if (error) throw error; } else { const { error } = await supabase.auth.signUp({ email: authEmail.trim(), password: authPassword }); if (error) throw error; setAuthMode("sign-in"); setAuthError("Account created. If email confirmation is enabled, confirm it before signing in."); } } catch (error) { setAuthError(error?.message || "Authentication failed."); } finally { setAuthLoading(false); } }
  async function handleSignOut() { if (!supabase) return; const { error } = await supabase.auth.signOut(); if (error) setAuthError(error.message || "Could not sign out."); }

  if (!session) return <AuthScreen mode={authMode} email={authEmail} password={authPassword} loading={authLoading} errorMessage={authError} onModeChange={setAuthMode} onEmailChange={setAuthEmail} onPasswordChange={setAuthPassword} onSubmit={handleAuthSubmit} />;

  return <div className="min-h-screen bg-slate-50 text-slate-900"><AppHeader installPromptEvent={installPromptEvent} onInstall={handleInstall} /><main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8"><div className="flex flex-wrap items-center justify-between gap-3"><nav className="grid grid-cols-3 gap-2 rounded-3xl border border-slate-200 bg-white p-2 shadow-sm w-full sm:w-fit">{[["today", "Today"], ["log", "History"], ["settings", "Settings"]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={classNames("rounded-2xl px-4 py-3 text-sm font-semibold transition", tab === key ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50")}>{label}</button>)}</nav><button onClick={handleSignOut} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Sign out</button></div>{tab === "today" ? <div className="mt-6 space-y-4"><OverviewCard regularState={regularState} prnState={prnState} lastRegular={lastRegular} lastTramadol={lastTramadol} paracetamolToday={paracetamolToday} settings={data.settings} effectiveNow={effectiveNow} onOpen={openSheet} /><RecentEventsPanel events={data.events} plans={data.settings.plans} onDelete={deleteEvent} /></div> : null}{tab === "log" ? <section className="mt-6"><HistoryPanel events={data.events} plans={data.settings.plans} filter={data.ui.historyFilter} onFilterChange={(historyFilter) => updateUI({ ...data.ui, historyFilter })} effectiveNow={effectiveNow} onDelete={deleteEvent} /></section> : null}{tab === "settings" ? <section className="mt-6"><SettingsPanel settings={data.settings} onChange={updateSettings} onReset={resetSettings} notificationPermission={notificationPermission} onRequestNotifications={requestNotifications} /></section> : null}</main><ActionSheet open={sheet.open} plans={data.settings.plans} events={data.events} settings={data.settings} effectiveNow={effectiveNow} sheet={sheet} onClose={closeSheet} onConfirm={addEvent} /></div>;
}
