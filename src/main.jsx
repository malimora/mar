...
const STORAGE_KEY = "med-schedule-pwa-v7";
...
function getRequiredPlanState(plan, events, now) { const lastTaken=getLatestTakenEventByPlan(events, plan.id, now); if(lastTaken){const nextDueAt=new Date(new Date(lastTaken.actualAt).getTime()+plan.intervalMinutes*60000); const delta=nextDueAt.getTime()-now.getTime(); return {plan,lastTaken,nextDueAt,status:delta<0?"overdue":delta<=1800000?"due-soon":"tracking"};} const reference=getReferenceScheduleState(plan, now); if(reference.previous){ return {plan,lastTaken:null,nextDueAt:reference.previous,status:now.getTime()>reference.previous.getTime()?"overdue":"due-soon"}; } return {plan,lastTaken:null,nextDueAt:reference.next,status:reference.next?"upcoming":"empty"}; }
function getPrimaryRequiredState(plans, events, now){ const required=(Array.isArray(plans)?plans:[]).filter((p)=>p.kind==="required"); const states=required.map((p)=>getRequiredPlanState(p, events, now)).filter((s)=>s.nextDueAt); if(!states.length) return null; const overdue=states.filter((s)=>s.status==="overdue").sort((a,b)=>a.nextDueAt-b.nextDueAt); if(overdue.length) return overdue[0]; return states.sort((a,b)=>a.nextDueAt-b.nextDueAt)[0]; }
...
function summarizeEvents(events, plans){ const planMap=getPlanMap(plans); const requiredIds=(plans||[]).filter((p)=>p.kind==="required").map((p)=>p.id); const takenRegular=events.filter((e)=>requiredIds.includes(e.planId)&&e.status==="taken").length; const takenPrn=events.filter((e)=>e.planId==="prn"&&e.status==="taken").length; const skippedRegular=events.filter((e)=>requiredIds.includes(e.planId)&&e.status==="skipped").length; const notNeededPrn=events.filter((e)=>e.planId==="prn"&&e.status==="not-needed").length; const paracetamolMg=events.filter((e)=>e.status==="taken").reduce((sum,e)=>sum+Number((planMap[e.planId]&&planMap[e.planId].paracetamolMg)||0),0); const highestPain=events.map((e)=>Number(e.painBefore)).filter((v)=>Number.isFinite(v)&&v>=0).sort((a,b)=>b-a)[0]; return {takenRegular,takenPrn,skippedRegular,notNeededPrn,paracetamolMg,highestPain}; }
...
function OverviewCard({ primaryRequiredState, prnState, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  const tone = primaryRequiredState?.status === "overdue" ? "border-rose-200 bg-rose-50/80" : "border-slate-200 bg-white";
  ...
  const overdueDuration = primaryRequiredState?.nextDueAt ? formatRelative(effectiveNow.getTime() - primaryRequiredState.nextDueAt.getTime()) : null;
  const primaryTitle = !primaryRequiredState ? "No regular dose logged yet" : primaryRequiredState.status === "overdue" ? `${primaryRequiredState.plan.label} overdue` : `Next ${primaryRequiredState.plan.label.toLowerCase()} at ${formatTime(primaryRequiredState.nextDueAt)}`;
  const primarySubtitle = !primaryRequiredState ? "Log the first regular dose to start the rolling schedule." : primaryRequiredState.status === "overdue" ? `${overdueDuration} late` : `In ${formatRelative(primaryRequiredState.nextDueAt.getTime() - effectiveNow.getTime())}`;
  const dueLine = primaryRequiredState?.status === "overdue" && primaryRequiredState?.nextDueAt ? `${primaryRequiredState.plan.medication} due since ${formatDateTime(primaryRequiredState.nextDueAt)}` : null;
  ...
  return <section className={classNames("rounded-3xl border p-5 shadow-sm sm:p-6", tone)}><div className="flex items-start justify-between gap-3"><h2 className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{primaryTitle}</h2><div className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200"><ClockIcon className="h-3.5 w-3.5" />Updated {formatTime(effectiveNow)}</div></div>{primaryRequiredState?.plan?.medication?<p className="mt-1 text-sm text-slate-600">Current overdue/next plan: {primaryRequiredState.plan.medication}</p>:null}<p className="mt-2 text-xl font-semibold text-slate-900">{primarySubtitle}</p>{dueLine?<p className="mt-1 text-base text-slate-700">{dueLine}</p>:null}<div className="mt-3 space-y-1 text-sm leading-6 text-slate-700"><div>{prnLine}</div><div>{safetyLine}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:max-w-xl"><button onClick={() => onOpen("regular-tramadol", "taken")} className="min-h-12 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">Log regular tramadol</button><button onClick={() => onOpen("regular-paracetamol", "taken")} className="min-h-12 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Log regular paracetamol</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><OverviewMetric label="Last tramadol" value={lastTramadol ? formatDateTime(lastTramadol.actualAt) : "Not logged"} /><OverviewMetric label="Breakthrough dose" value={prnState.available ? "Available now" : `In ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}`} /><OverviewMetric label="Paracetamol since midnight" value={`${paracetamolToday} mg`} /></div></section>;
}
...
function MedicationSchedulePWA() {
  ...
  const effectiveNow = liveNow; const planMap = useMemo(() => getPlanMap(data.settings.plans), [data.settings.plans]); const prnPlan = planMap.prn; const primaryRequiredState = useMemo(() => getPrimaryRequiredState(data.settings.plans, data.events, effectiveNow), [data.settings.plans, data.events, effectiveNow]); const prnState = useMemo(() => getPrnPlanState(prnPlan, data.events, data.settings, effectiveNow), [prnPlan, data.events, data.settings, effectiveNow]); const lastTramadol = useMemo(() => getLatestTramadolEvent(data.events, effectiveNow), [data.events, effectiveNow]); const paracetamolToday = useMemo(() => getParacetamolToday(data.events, data.settings.plans, effectiveNow), [data.events, data.settings.plans, effectiveNow]);
