...
function getCareDayStart(dateLike){ const d=new Date(dateLike); const s=new Date(d); s.setHours(6,0,0,0); if(d<s){ s.setDate(s.getDate()-1);} return s; }
function getCareDayEnd(dateLike){ const s=getCareDayStart(dateLike); const e=new Date(s); e.setDate(e.getDate()+1); return e; }
function formatSignedMinutes(totalMinutes){ if(totalMinutes==null||!Number.isFinite(totalMinutes)) return "—"; const sign=totalMinutes<0?"-":""; const abs=Math.abs(totalMinutes); const h=Math.floor(abs/60); const m=abs%60; return h?`${sign}${h}h ${pad(m)}m`:`${sign}${m}m`; }
function medCode(planId){ if(planId==="regular-paracetamol") return "P"; if(planId==="regular-tramadol") return "T"; if(planId==="prn") return "▲"; return "•"; }
function generatePlannedSlots(plans, careDayStart){ const order=["06:00","12:00","18:00","00:00"]; const slots=[]; plans.filter((p)=>p.kind==="required").forEach((plan)=>{(plan.baseTimes||[]).forEach((t,idx)=>{const m=parseClockTime(t); if(m==null) return; const at=new Date(careDayStart); if(m<360) at.setDate(at.getDate()+1); at.setHours(Math.floor(m/60),m%60,0,0); slots.push({id:`${plan.id}-${idx}-${t}`,scheduledAt:at,medicationCode:plan.id.includes('tramadol')?'tramadol':'paracetamol',doseMg:plan.id.includes('tramadol')?plan.tramadolMg:plan.paracetamolMg,planType:'regular',roundLabel:t,planId:plan.id});});}); return slots.sort((a,b)=>order.indexOf(a.roundLabel)-order.indexOf(b.roundLabel)||a.scheduledAt-b.scheduledAt); }
function getSlotStatus(slot,event,now){ if(event) return 'taken'; const delta=(now-slot.scheduledAt)/60000; if(delta<0) return 'upcoming'; if(delta<=30) return 'due'; if(delta<=120) return 'late'; return 'missed'; }
function getDeviationMinutes(slot,event){ if(!event) return null; return Math.round((new Date(event.actualAt)-slot.scheduledAt)/60000); }
function matchActualEventsToPlannedSlots(events, plannedSlots){ const regular=events.filter((e)=>e.status==='taken'&&e.planId!=='prn'); const used=new Set(); const matched=plannedSlots.map((slot)=>{ let best=null,bestDiff=1e12,bestIdx=-1; regular.forEach((e,idx)=>{ if(used.has(idx)||e.planId!==slot.planId) return; const diff=(new Date(e.actualAt)-slot.scheduledAt)/60000; if(diff<-90||diff>180) return; const a=Math.abs(diff); if(a<bestDiff){best=e;bestDiff=a;bestIdx=idx;} }); if(bestIdx>=0) used.add(bestIdx); return {slot,event:best};});
 const extras=regular.filter((_,idx)=>!used.has(idx)); return {matched,extras}; }
function getPainPoints(events,start,end){ return events.filter((e)=>new Date(e.actualAt)>=start&&new Date(e.actualAt)<=end).flatMap((e)=>{const pts=[]; if(Number.isFinite(Number(e.painBefore))) pts.push({t:new Date(e.actualAt),pain:Number(e.painBefore),planId:e.planId}); if(Number.isFinite(Number(e.painAfter))) pts.push({t:new Date(new Date(e.actualAt).getTime()+90*60000),pain:Number(e.painAfter),planId:e.planId}); return pts; }).sort((a,b)=>a.t-b.t); }
function getDoseImpact(events,start,end){ const inRange=events.filter((e)=>e.status==='taken'&&new Date(e.actualAt)>=start&&new Date(e.actualAt)<=end); const painLogs=inRange.flatMap((e)=>[{t:new Date(e.actualAt),pain:Number(e.painBefore),valid:Number.isFinite(Number(e.painBefore))}]).filter(x=>x.valid);
 const groups={'regular-tramadol':[], 'prn':[], 'regular-paracetamol':[]}; inRange.forEach((e)=>{const before=Number.isFinite(Number(e.painBefore))?Number(e.painBefore):null; if(before==null) return; const et=new Date(e.actualAt).getTime(); const after=painLogs.find((p)=>p.t.getTime()>et+60*60000&&p.t.getTime()<=et+4*3600000); if(!after) return; groups[e.planId]?.push({before,after:after.pain});});
 return groups; }
function getBreakthroughStats(events,start,end){ const list=events.filter((e)=>e.planId==='prn'&&e.status==='taken'&&new Date(e.actualAt)>=start&&new Date(e.actualAt)<=end); const byDay={}; list.forEach((e)=>{const key=formatDate(startOfDay(e.actualAt)); byDay[key]=(byDay[key]||0)+1;}); const windows={morning:0,afternoon:0,evening:0,night:0}; list.forEach((e)=>{const h=new Date(e.actualAt).getHours(); if(h<6) windows.night++; else if(h<12) windows.morning++; else if(h<18) windows.afternoon++; else windows.evening++;}); const top=Object.entries(windows).sort((a,b)=>b[1]-a[1])[0]; const pains=list.map((e)=>Number(e.painBefore)).filter(Number.isFinite); return {list,byDay,avg:pains.length?pains.reduce((a,b)=>a+b,0)/pains.length:null,top:top&&top[1]>0?top[0]:null}; }
function getTimingWarnings(events,plans,now){ const last24=new Date(now.getTime()-24*3600000); const planMap=getPlanMap(plans); const taken=events.filter((e)=>e.status==='taken'&&new Date(e.actualAt)>=last24&&new Date(e.actualAt)<=now); const by=(key)=>taken.filter((e)=>key(e)).sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt)); const pg=by((e)=>Number(planMap[e.planId]?.paracetamolMg)>0); const tg=by((e)=>Number(planMap[e.planId]?.tramadolMg)>0); const minGap=(arr)=>{let m=null; for(let i=1;i<arr.length;i++){const g=(new Date(arr[i].actualAt)-new Date(arr[i-1].actualAt))/60000; if(m==null||g<m)m=g;} return m;}; return {pGap:minGap(pg), tGap:minGap(tg)}; }
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) { const planMap=getPlanMap(plans); const careStart=getCareDayStart(effectiveNow); const careEnd=getCareDayEnd(effectiveNow); const slots=generatePlannedSlots(plans, careStart); const {matched}=matchActualEventsToPlannedSlots(events.filter((e)=>new Date(e.actualAt)>=careStart&&new Date(e.actualAt)<careEnd), slots);
 const rows=matched.map((m)=>({ ...m, status:getSlotStatus(m.slot,m.event,effectiveNow), deviationMinutes:getDeviationMinutes(m.slot,m.event) }));
 const painPoints=getPainPoints(events, getHistoryFilterStart(filter,effectiveNow)||careStart, effectiveNow);
 const impact=getDoseImpact(events, getHistoryFilterStart(filter,effectiveNow)||careStart, effectiveNow);
 const bt=getBreakthroughStats(events, getHistoryFilterStart(filter,effectiveNow)||careStart, effectiveNow);
 const tw=getTimingWarnings(events,plans,effectiveNow); const medsLogged=rows.filter((r)=>r.event).length; const next=rows.find((r)=>['upcoming','due','late'].includes(r.status)); const last=[...events].filter((e)=>e.status==='taken'&&new Date(e.actualAt)>=careStart&&new Date(e.actualAt)<careEnd).sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt))[0];
 const round06=rows.filter((r)=>r.slot.roundLabel==='06:00'); const phrase=round06.every((r)=>r.event)?'Morning round complete':round06.some((r)=>r.event)?'Morning round partial':'On track';
 const xFor=(d)=>((new Date(d)-careStart)/(careEnd-careStart))*100;
 const painPath=painPoints.map((p,i)=>`${i?'L':'M'} ${xFor(p.t)} ${100-(p.pain/10)*100}`).join(' ');
 return <div className="space-y-4"><section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold text-slate-700">Insights range</h2><div className="flex gap-2">{[["today","Today"],["3d","3 days"],["7d","7 days"],["all","All"]].map(([k,l])=><button key={k} onClick={()=>onFilterChange(k)} className={classNames("rounded-full px-3 py-1 text-sm",filter===k?"bg-slate-900 text-white":"bg-slate-100 text-slate-700")}>{l}</button>)}</div></div></section><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Today</h2><div className="mt-1 text-sm text-slate-600">{phrase}</div><div className="mt-3 flex items-center gap-3"><div className="h-14 w-14 rounded-full border-4 border-slate-200 grid place-items-center text-xs">{medsLogged}/8</div><div className="text-sm"><div>Next: {next?`${medCode(next.slot.planId)} ${formatTime(next.slot.scheduledAt)}`:'No remaining planned doses'}</div><div>Last: {last?`${medCode(last.planId)} ${formatTime(last.actualAt)}`:'Not logged'}</div><div>{painPoints.length?'':'Pain data missing'}</div></div></div></section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm overflow-x-auto"><h2 className="text-lg font-semibold">Medication timeline</h2><svg viewBox="0 0 1000 180" className="mt-2 min-w-[700px] w-full h-44"><line x1="40" y1="40" x2="960" y2="40" stroke="#cbd5e1"/><line x1="40" y1="90" x2="960" y2="90" stroke="#cbd5e1"/><line x1="40" y1="140" x2="960" y2="140" stroke="#cbd5e1"/>{[6,12,18,24].map((h,i)=>{const x=40+(h/24)*920; return <g key={h}><line x1={x} y1="24" x2={x} y2="156" stroke="#e2e8f0"/><text x={x-12} y="18" fontSize="11">{i===3?'12 AM':h===12?'12 PM':'6 AM'}</text></g>;})}<line x1={40+xFor(effectiveNow)*9.2} y1="20" x2={40+xFor(effectiveNow)*9.2} y2="160" stroke="#0f172a" strokeWidth="1"/>{rows.map((r)=>{const y=r.slot.planId.includes('tramadol')?40:90; const sx=40+xFor(r.slot.scheduledAt)*9.2; if(r.event){const ax=40+xFor(r.event.actualAt)*9.2; return <g key={r.slot.id}><circle cx={sx} cy={y} r="5" fill="white" stroke="#94a3b8"/><line x1={sx} y1={y} x2={ax} y2={y} stroke="#f59e0b"/><circle cx={ax} cy={y} r="5" fill="#0f172a"/></g>;} if(r.status==='missed') return <text key={r.slot.id} x={sx-4} y={y+4} fill="#dc2626">×</text>; return <circle key={r.slot.id} cx={sx} cy={y} r="5" fill="white" stroke="#94a3b8"/>;})}{events.filter((e)=>e.planId==='prn'&&new Date(e.actualAt)>=careStart&&new Date(e.actualAt)<careEnd).map((e,i)=><text key={i} x={40+xFor(e.actualAt)*9.2-4} y="145" fill="#7c3aed">▲</text>)}</svg><div className="text-xs text-slate-500">● taken ○ planned × missed ▲ breakthrough</div></section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Timing drift</h2><div className="mt-2 space-y-2">{rows.map((r)=><div key={r.slot.id} className="text-sm"><div className="flex justify-between"><span>{formatTime(r.slot.scheduledAt)} {medCode(r.slot.planId)}</span><span>{r.event?`${r.deviationMinutes>0?'+':''}${r.deviationMinutes}m ${Math.abs(r.deviationMinutes)<=15?'on time':'late'}`:r.status}</span></div>{r.event?<div className="h-1.5 bg-slate-100 rounded"><div className="h-1.5 bg-amber-400 rounded" style={{width:`${Math.min(100,Math.abs(r.deviationMinutes)/120*100)}%`}}/></div>:null}</div>)}</div></section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm overflow-x-auto"><h2 className="text-lg font-semibold">Pain trend</h2>{painPoints.length===0?<div className="mt-2 text-sm text-slate-500">No pain scores logged for this range.</div>:<svg viewBox="0 0 1000 180" className="mt-2 min-w-[700px] w-full h-44"><text x="8" y="20" fontSize="10">10</text><text x="8" y="92" fontSize="10">5</text><text x="8" y="164" fontSize="10">0</text><path d={painPath} fill="none" stroke="#0f172a" strokeWidth="2"/>{painPoints.map((p,i)=><circle key={i} cx={40+xFor(p.t)*9.2} cy={10+(100-(p.pain/10)*100)*1.5} r="3" fill="#0f172a"/> )}</svg>}</section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Pain change after dose</h2>{Object.entries({'regular-tramadol':'Regular Tramadol','prn':'Breakthrough','regular-paracetamol':'Paracetamol'}).map(([k,label])=>{const arr=impact[k]||[]; if(!arr.length) return <div key={k} className="mt-2 text-sm text-slate-500">{label}: Not enough paired data</div>; const b=arr.reduce((s,x)=>s+x.before,0)/arr.length; const a=arr.reduce((s,x)=>s+x.after,0)/arr.length; return <div key={k} className="mt-2 text-sm">{label}: {b.toFixed(1)} → {a.toFixed(1)} ({(a-b).toFixed(1)}) n={arr.length}</div>;})}</section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Breakthrough use</h2>{Object.keys(bt.byDay).length===0?<div className="text-sm text-slate-500">No breakthrough doses logged in this range.</div>:Object.entries(bt.byDay).map(([d,c])=><div key={d} className="text-sm">{d} {'█'.repeat(c)} {c}</div>)}<div className="mt-2 text-sm text-slate-600">Avg pain before breakthrough: {bt.avg==null?'—':bt.avg.toFixed(1)+' / 10'}</div><div className="text-sm text-slate-600">Most common time: {bt.top||'—'}</div></section>
 <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Timing checks</h2><div className="text-sm">Paracetamol shortest gap: {tw.pGap==null?'—':formatSignedMinutes(Math.round(tw.pGap)).replace('+','')}</div><div className="text-sm">Tramadol shortest gap: {tw.tGap==null?'—':formatSignedMinutes(Math.round(tw.tGap)).replace('+','')}</div><div className="mt-2 text-sm text-slate-600">{tw.pGap!=null&&tw.pGap<240?'Paracetamol doses were close together. Check your discharge instructions before taking more.':'No timing warnings.'}</div></section>
 <details className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><summary className="cursor-pointer font-semibold">Log</summary><div className="mt-2 space-y-1">{[...events].sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt)).slice(0,24).map((e)=><div key={e.id} className="flex justify-between text-sm"><span>{formatDate(new Date(e.actualAt))} {formatTime(e.actualAt)} {medCode(e.planId)} {planMap[e.planId]?.medication||e.planId}</span><button onClick={()=>onDelete(e.id)} className="text-slate-400">✕</button></div>)}</div></details></div>; }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const payload = { id: 1, tramadol_spacing_minutes: data.settings.tramadolSpacingMinutes, reminders: data.settings.reminders, history_filter: data.ui.historyFilter };
