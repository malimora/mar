...
function TodayDashboard({ events, plans, prnState, effectiveNow, onOpen, onDelete }) {
  ...
  const upcomingTracks = regularPlans.map((plan) => {
    const state = getRegularPlanState(plan, events, effectiveNow);
    return { plan, dueAt: state.nextDueAt || effectiveNow, state };
  ...
  const perPlanToday = regularPlans.map((plan) => {
    const taken = todayEvents.filter((e) => e.planId === plan.id && e.status === "taken");
    const state = getRegularPlanState(plan, events, effectiveNow);
    const plannedToday = Math.max(taken.length + (new Date(state.nextDueAt || effectiveNow).getDate() === effectiveNow.getDate() ? 1 : 0), plan.baseTimes?.length || 1);
    return { plan, taken, nextDueAt: state.nextDueAt, plannedToday };
  });
  const perPlanTiming = perPlanToday.map(({ plan, taken }) => {
    const deltas = taken.slice(1).map((e, i) => new Date(e.actualAt).getTime() - new Date(taken[i].actualAt).getTime() - (plan.intervalMinutes * 60000));
    const avg = deltas.length ? Math.round(deltas.reduce((a,b)=>a+b,0)/deltas.length/60000) : null;
    return { plan, avg };
  });
  return <div className="space-y-4">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-medium">Upcoming medications</p><div className="mt-3 space-y-3">{upcomingTracks.map((item)=><div key={item.plan.id} className="rounded-2xl border border-slate-200 p-3"><div className="text-lg font-semibold">{formatTime(item.dueAt)}</div><div className="text-sm font-medium">{item.plan.medication}</div><div className="mt-1 text-xs text-slate-600">{new Date(item.dueAt).getTime()<=effectiveNow.getTime()?"Due now":`Due in ${formatRelative(new Date(item.dueAt).getTime()-effectiveNow.getTime())}`}</div><button onClick={()=>onOpen(item.plan.id,"taken")} className="mt-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white">{item.plan.medication.includes("Tramadol")?"Log Tramadol":"Log Paracetamol"}</button></div>)}</div></section>
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="font-semibold">Today’s regular medications</h3><div className="mt-4 space-y-3">{perPlanToday.map(({ plan, taken, nextDueAt })=><div key={plan.id} className="rounded-2xl border border-slate-200 p-3"><div className="text-sm font-semibold">{plan.medication}</div><div className="text-xs text-slate-600">Interval: every {plan.intervalMinutes} minutes</div><div className="text-xs text-slate-600">Last taken: {taken.length?formatTime(taken[taken.length-1].actualAt):"No logs yet today"}</div><div className="text-xs text-slate-600">Next due: {nextDueAt?formatTime(nextDueAt):"—"}</div><div className="mt-2 flex flex-wrap gap-2">{taken.map((e)=><span key={e.id} className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-xs text-emerald-700">{formatTime(e.actualAt)} taken</span>)}</div></div>)}</div></section>
    <div className="grid gap-4 lg:grid-cols-3">
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-semibold">Regular medications</h3>{regularPlans.map((plan)=><div key={plan.id} className="mt-2 text-sm"><div>{plan.medication}</div><div className="text-slate-600">Every {Math.round(plan.intervalMinutes/60)} hours</div><div className="text-slate-600">{plan.baseTimes.join(" · ")}</div></div>)}</section>
      <section className="rounded-3xl border border-indigo-200 bg-indigo-50/60 p-4 shadow-sm"><h3 className="font-semibold text-indigo-900">As-needed medication</h3><p className="mt-2 text-sm">Tramadol 50 mg</p><p className="text-sm text-indigo-700">Optional breakthrough medication</p><p className="mt-2 text-sm text-indigo-700">{prnState.available?"Available now":`Available from ${formatTime(prnState.availableAt)}`}</p><p className="mt-1 text-sm text-indigo-700">Used today: {events.filter((e)=>e.planId==="prn"&&e.status==="taken"&&isSameDay(e.actualAt,effectiveNow)).length}</p><button onClick={()=>onOpen('prn','taken')} className="mt-3 rounded-xl bg-indigo-700 px-3 py-2 text-xs font-semibold text-white">Log PRN Tramadol</button></section>
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-semibold">Regular progress</h3>{perPlanToday.map(({ plan, taken, plannedToday })=>{const denom=Math.max(1, plannedToday); return <div key={plan.id} className="mt-2"><p className="text-sm">{plan.medication.includes('Tramadol')?'Tramadol':'Paracetamol'} · {taken.length} / {denom} taken</p><div className="mt-1 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-500" style={{width:`${Math.min(100,(taken.length/denom)*100)}%`}} /></div></div>})}</section>
    </div>
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="font-semibold">Timing deviation</h3>{perPlanTiming.map(({ plan, avg }) => <p key={plan.id} className="mt-2 text-sm">{plan.medication.includes("Tramadol") ? "Tramadol" : "Paracetamol"}: {avg==null?"Not enough data yet":avg<0?`Usually ${Math.abs(avg)} min early`:avg>0?`Usually ${avg} min late`:"On time today"}</p>)}</section>
...
function MedicationSchedulePWA() {
  ...
  function applySettings(nextSettings) { setData((current) => ({ ...current, settings: { ...current.settings, ...nextSettings, reminders: { ...current.settings.reminders, ...((nextSettings && nextSettings.reminders) || {}) }, plans: Array.isArray(nextSettings && nextSettings.plans) ? normalizePlans(nextSettings.plans) : current.settings.plans } })); }
  async function saveSettings(nextSettings, options = {}) { const persist = options.persist !== false; applySettings(nextSettings); if (!persist) return; if (!supabase) return; try { setSettingsSaveStatus("saving"); const { data: authData } = await supabase.auth.getUser(); const userId = authData?.user?.id; if (!userId) { logSupabase("settings:sync:skipped", { reason: "no-auth-user" }); setSettingsSaveStatus("error"); return; } const normalized = normalizePlans((nextSettings && nextSettings.plans) || data.settings.plans); const planRows = normalized.map((plan) => mapPlanToUserPlanRow(plan)).filter(Boolean); if (planRows.length) { const { error: plansError } = await supabase.from("user_plans").upsert(planRows, { onConflict: "plan_id" }); if (plansError) throw plansError; } const payload = { id: 1, tramadol_spacing_minutes: (nextSettings?.tramadolSpacingMinutes ?? data.settings.tramadolSpacingMinutes), reminders: { ...data.settings.reminders, ...((nextSettings && nextSettings.reminders) || {}) } }; const { error: settingsError } = await supabase.from("user_settings").upsert(payload, { onConflict: "id" }); if (settingsError) throw settingsError; logSupabase("settings:sync:success", { planCount: planRows.length }); setSettingsSaveStatus("saved"); if (typeof window !== "undefined") window.setTimeout(() => setSettingsSaveStatus("idle"), 2000); } catch (error) { logSupabaseError("settings:sync", error); setSettingsSaveStatus("error"); } }
