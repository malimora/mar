...
function eventHasMedication(event, code){ return Array.isArray(event.medications)&&event.medications.some((m)=>m.code===code); }
function getLatestTramadolEvent(events, until) { const limit=until?new Date(until).getTime():null; return [...(Array.isArray(events)?events:[])].filter((e)=>e.status==="taken"&&eventHasMedication(e,"tramadol")).filter((e)=>(limit===null?true:new Date(e.actualAt).getTime()<=limit)).sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt)).slice(-1)[0]||null; }
...
function summarizeEvents(events, plans){ const planMap=getPlanMap(plans); const takenRegular=events.filter((e)=>e.planId==="regular"&&e.status==="taken").length; const takenPrn=events.filter((e)=>e.planId==="prn"&&e.status==="taken").length; const skippedRegular=events.filter((e)=>e.planId==="regular"&&e.status==="skipped").length; const notNeededPrn=events.filter((e)=>e.planId==="prn"&&e.status==="not-needed").length; const paracetamolMg=events.filter((e)=>e.status==="taken").reduce((sum,e)=>sum+(Array.isArray(e.medications)?e.medications.filter((m)=>m.code==="paracetamol").reduce((inner,m)=>inner+Number(m.doseMg||0),0):0),0); const highestPain=events.map((e)=>Number(e.painBefore)).filter((v)=>Number.isFinite(v)&&v>=0).sort((a,b)=>b-a)[0]; return {takenRegular,takenPrn,skippedRegular,notNeededPrn,paracetamolMg,highestPain}; }
...
function getSpacingWarning(events, includeTramadol, candidateAt, spacingMinutes){ if(!includeTramadol) return null; const previous=getLatestTramadolEvent(events, candidateAt); if(!previous) return null; const candidateMs=new Date(candidateAt).getTime(); const previousMs=new Date(previous.actualAt).getTime(); if(!Number.isFinite(candidateMs)||!Number.isFinite(previousMs)) return null; const elapsed=candidateMs-previousMs; if(elapsed<0||elapsed>=spacingMinutes*60000) return null; return {previous, elapsed}; }
function getParacetamolToday(events, plans, now){ const dayStart=new Date(now); dayStart.setHours(0,0,0,0); return events.filter((e)=>e.status==="taken").filter((e)=>{const t=new Date(e.actualAt).getTime(); return t>=dayStart.getTime()&&t<=now.getTime();}).reduce((sum,e)=>sum+(Array.isArray(e.medications)?e.medications.filter((m)=>m.code==="paracetamol").reduce((inner,m)=>inner+Number(m.doseMg||0),0):0),0); }
...
function OverviewCard({ regularState, prnState, lastRegular, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  const tone = regularState.status === "overdue" ? "border-rose-200 bg-rose-50/80" : "border-slate-200 bg-white";
  ...
  const primaryTitle = regularState.status === "empty" ? "No regular dose logged yet" : regularState.status === "overdue" ? `Regular overdue by ${formatRelative(effectiveNow.getTime() - regularState.nextDueAt.getTime())}` : `Next regular at ${formatTime(regularState.nextDueAt)}`;
  const primarySubtitle = regularState.status === "empty" ? "Log the first regular dose to start the rolling schedule." : regularState.status === "overdue" ? `Due since ${formatDateTime(regularState.nextDueAt)}` : `In ${formatRelative(regularState.nextDueAt.getTime() - effectiveNow.getTime())}`;
  const prnLine = prnState.available ? "Breakthrough dose available now if pain needs it." : `Breakthrough dose available in ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}.`;
  const safetyLine = nextTramadolSafeAt ? (effectiveNow.getTime() >= nextTramadolSafeAt.getTime() ? "Tramadol timing is clear now." : `Tramadol timing clears at ${formatTime(nextTramadolSafeAt)}.`) : "No Tramadol dose has been logged yet.";
  return <section className={classNames("rounded-3xl border p-5 shadow-sm sm:p-6", tone)}><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-2xl"><div className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200"><ClockIcon className="h-3.5 w-3.5" />Updated {formatTime(effectiveNow)}</div><h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{primaryTitle}</h2><p className="mt-1 text-lg text-slate-700">{primarySubtitle}</p><div className="mt-4 space-y-1 text-sm leading-6 text-slate-600"><div>{prnLine}</div><div>{safetyLine}</div></div></div><div className="grid w-full gap-3 sm:grid-cols-2 lg:w-[24rem]"><OverviewMetric label="Last regular" value={lastRegular ? formatTime(lastRegular.actualAt) : "Not logged"} /><OverviewMetric label="Last Tramadol" value={lastTramadol ? formatTime(lastTramadol.actualAt) : "Not logged"} /><OverviewMetric label="Breakthrough dose" value={prnState.available ? "Available now" : `In ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}`} /><OverviewMetric label="Paracetamol today" value={`${paracetamolToday}mg`} /></div></div><div className="mt-5 grid gap-3 md:grid-cols-3"><button onClick={() => onOpen("regular", "taken")} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white">Log regular dose</button><button onClick={() => onOpen("prn", "taken")} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Log as-needed dose</button><button onClick={() => onOpen("prn", "not-needed")} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Mark PRN not needed</button></div></section>;
}

function RecentEventsPanel({ events, plans, onDelete }) { const planMap = getPlanMap(plans); const recent=[...events].sort((a,b)=>new Date(b.actualAt)-new Date(a.actualAt)).slice(0,6); return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div><h2 className="text-lg font-semibold text-slate-900">Recent activity</h2><p className="mt-1 text-sm text-slate-600">Multi-day history stays on this device for ongoing tracking.</p></div><div className="mt-4 space-y-3">{recent.length===0?<div className="rounded-2xl border border-dashed border-slate-200 p-5 text-sm text-slate-500">No events logged yet.</div>:recent.map((event)=><div key={event.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-sm font-semibold text-slate-900">{(planMap[event.planId]&&planMap[event.planId].label)||event.planId} · {formatStatus(event.status)}</div><div className="mt-1 text-sm text-slate-600">{(planMap[event.planId]&&planMap[event.planId].medication)||"Unknown medication"}</div><div className="mt-1 text-xs text-slate-500">{formatDateTime(event.actualAt)}</div></div><div className="flex items-center gap-3"><div className="text-right text-sm text-slate-600">{Number.isFinite(Number(event.painBefore))?<div>Pain before: {event.painBefore}/10</div>:null}{Number.isFinite(Number(event.painAfter))?<div>Pain after: {event.painAfter}/10</div>:null}</div><button onClick={() => onDelete(event.id)} className="rounded-2xl border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-50"><TrashIcon className="h-4 w-4" /></button></div></div>{event.note?<div className="mt-3 whitespace-pre-wrap text-sm text-slate-600">{event.note}</div>:null}</div>)}</div></section>; }
...
function ActionSheet({ open, plans, events, settings, effectiveNow, sheet, onClose, onConfirm }) {
  ...
  const [regularParacetamol, setRegularParacetamol] = useState(true);
  const [regularParacetamolDose, setRegularParacetamolDose] = useState("500");
  const [regularTramadol, setRegularTramadol] = useState(true);
  const [regularTramadolDose, setRegularTramadolDose] = useState("15");
  const [prnTramadolDose, setPrnTramadolDose] = useState("15");
  useEffect(() => { if (!open) return; setStatus(sheet.status || "taken"); setActualAt(toLocalInputValue(effectiveNow)); setPainBefore(""); setPainAfter(""); setNote(""); setRegularParacetamol(true); setRegularParacetamolDose("500"); setRegularTramadol(true); setRegularTramadolDose("15"); setPrnTramadolDose("15"); }, [open, sheet.status, sheet.planId, effectiveNow]);
  if (!open || !plan) return null;
  const actualDate = fromLocalInputValue(actualAt);
  const selectedMeds = status!=="taken"?[]:(plan.id==="prn"?[{code:"tramadol",doseMg:Number(prnTramadolDose||0)}]:[{code:"paracetamol",doseMg:Number(regularParacetamolDose||0),enabled:regularParacetamol},{code:"tramadol",doseMg:Number(regularTramadolDose||0),enabled:regularTramadol}].filter((m)=>m.enabled).map((m)=>({code:m.code,doseMg:m.doseMg})));
  const spacingWarning = status === "taken" && actualDate ? getSpacingWarning(events, selectedMeds.some((m)=>m.code==="tramadol"), actualDate, settings.tramadolSpacingMinutes) : null;
  const statusOptions = plan.kind === "required" ? [["taken", "Taken"], ["skipped", "Skipped"]] : [["taken", "Taken"], ["not-needed", "Not needed"], ["skipped", "Skipped"]];
  return <div className="fixed inset-0 z-30 flex items-end justify-center bg-slate-900/40 p-2 sm:p-3 sm:items-center"><div className="w-full max-w-lg rounded-3xl bg-white p-4 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><h3 className="text-xl font-semibold text-slate-900">Log {plan.label.toLowerCase()} event</h3><p className="mt-1 text-sm text-slate-600">{plan.medication}</p></div><button onClick={onClose} className="rounded-2xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700">Close</button></div>{spacingWarning?<div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><div className="font-semibold">Tramadol timing warning</div><div className="mt-1">A Tramadol event was logged {formatRelative(spacingWarning.elapsed)} earlier at {formatDateTime(spacingWarning.previous.actualAt)}.</div></div>:null}<div className="mt-4 grid gap-4"><Field label="Status"><select value={status} onChange={(event) => setStatus(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-slate-300">{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Actual date and time"><input type="datetime-local" value={actualAt} onChange={(event) => setActualAt(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-slate-300" /></Field>{status === "taken" || plan.kind === "optional" ? <Field label="Pain before (optional)"><input type="number" min={0} max={10} step={1} value={painBefore} onChange={(event) => setPainBefore(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="0 to 10" /></Field> : null}{status === "taken" ? <Field label="Pain after (optional)"><input type="number" min={0} max={10} step={1} value={painAfter} onChange={(event) => setPainAfter(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="0 to 10" /></Field> : null}{status === "taken" ? <div className="rounded-2xl border border-slate-200 p-3"><div className="mb-2 text-sm font-medium text-slate-700">Medications</div>{plan.id === "regular" ? <div className="space-y-3"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={regularParacetamol} onChange={(e)=>setRegularParacetamol(e.target.checked)} />Paracetamol</label>{regularParacetamol ? <input type="number" min={1} step={1} value={regularParacetamolDose} onChange={(e)=>setRegularParacetamolDose(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Dose mg" /> : null}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={regularTramadol} onChange={(e)=>setRegularTramadol(e.target.checked)} />Tramadol</label>{regularTramadol ? <input type="number" min={1} step={1} value={regularTramadolDose} onChange={(e)=>setRegularTramadolDose(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Dose mg" /> : null}</div> : <div><div className="text-sm text-slate-600 mb-2">PRN uses Tramadol.</div><input type="number" min={1} step={1} value={prnTramadolDose} onChange={(e)=>setPrnTramadolDose(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Tramadol dose mg" /></div>}</div> : null}<Field label={plan.kind === "optional" && status === "taken" ? "Why taking this? (optional)" : "Note (optional)"}><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={4} className="w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder={plan.kind === "optional" && status === "taken" ? "Pain flare after physio, night pain, swelling..." : "Anything useful to remember"} /></Field><div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">The time defaults to now, but you can override it here for historical entries or corrections.</div></div><div className="mt-5 flex gap-3"><button onClick={() => { const parsedDate = fromLocalInputValue(actualAt); if (!parsedDate) { alert("Please enter a valid date and time."); return; } if(status==="taken" && selectedMeds.length===0){ alert("Select at least one medication."); return; } if(status==="taken" && selectedMeds.some((m)=>!Number.isFinite(Number(m.doseMg))||Number(m.doseMg)<=0)){ alert("Dose (mg) must be greater than 0 for each selected medication."); return; } onConfirm({ planId: plan.id, status, actualAt: parsedDate.toISOString(), painBefore: parsePainValue(painBefore), painAfter: parsePainValue(painAfter), note: String(note || "").trim(), medications:selectedMeds }); }} className="flex-1 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Save event</button><button onClick={onClose} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700">Cancel</button></div></div></div>;
}
...
function mapDbEventToApp(row) {
  return {
    ...
    medications: Array.isArray(row.dose_event_items)?row.dose_event_items.map((item)=>({code:item.medication_code,doseMg:Number(item.dose_mg)})):[],
    containsTramadol: Array.isArray(row.dose_event_items)?row.dose_event_items.some((item)=>item.medication_code==="tramadol"):false,
  };
}
...
function MedicationSchedulePWA() {
  ...
  async function addEvent(payload) { const plan = planMap[payload.planId]; if (!plan) return; const nextEvent = { id: crypto.randomUUID(), planId: payload.planId, status: payload.status, actualAt: new Date(payload.actualAt).toISOString(), note: payload.note || "", painBefore: payload.painBefore, painAfter: payload.painAfter, createdAt: new Date().toISOString(), medications: payload.status==="taken"?(payload.medications||[]):[], containsTramadol: (payload.medications||[]).some((m)=>m.code==="tramadol") }; setData((current) => ({ ...current, events: normalizeEvents([...current.events, nextEvent], current.settings.plans) })); if (supabase) { try { const dbEvent = mapAppEventToDb(nextEvent); logSupabase("event:insert:attempt", { eventId: dbEvent.id, planId: dbEvent.plan_id, status: dbEvent.status, actualAt: dbEvent.actual_at }); const { error } = await supabase.from("dose_events").insert(dbEvent); if (error) throw error; if(nextEvent.status==="taken"&&nextEvent.medications.length){const itemRows=nextEvent.medications.map((m)=>({dose_event_id:nextEvent.id,medication_code:m.code,dose_mg:m.doseMg})); const { error:itemError } = await supabase.from("dose_event_items").insert(itemRows); if(itemError) throw itemError;} logSupabase("event:insert:success", { eventId: dbEvent.id }); } catch (error) { logSupabaseError("event:insert", error, { eventId: nextEvent.id }); } } else { logSupabase("event:insert:skipped", { reason: "missing supabase client", hasSupabaseClient: Boolean(supabase), eventId: nextEvent.id }); } closeSheet(); }
  async function deleteEvent(eventId) { if (isBrowser() && typeof window.confirm === "function") { const confirmed = window.confirm("Delete this event?"); if (!confirmed) return; } setData((current) => ({ ...current, events: current.events.filter((event) => event.id !== eventId) })); if (supabase) { try { logSupabase("event:delete:attempt", { eventId }); const { error } = await supabase.from("dose_events").delete().eq("id", eventId); if (error) throw error; logSupabase("event:delete:success", { eventId }); } catch (error) { logSupabaseError("event:delete", error, { eventId }); } } else { logSupabase("event:delete:skipped", { reason: "missing supabase client", hasSupabaseClient: Boolean(supabase), eventId }); } }
