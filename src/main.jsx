...
function getCareDayStart(date){ const d=new Date(date); if(Number.isNaN(d.getTime())) return null; const start=new Date(d); start.setHours(6,0,0,0); if(d.getHours()<6) start.setDate(start.getDate()-1); return start; }
function getCareDayEnd(date){ const start=getCareDayStart(date); if(!start) return null; return new Date(start.getTime()+24*60*60*1000); }
function generatePlannedSlots(planConfig, careDayStart){ const slots=[]; (planConfig||[]).filter((p)=>p.kind==='required').forEach((plan)=>{(plan.baseTimes||[]).forEach((t)=>{const mins=parseClockTime(t); if(mins===null) return; const d=new Date(careDayStart); d.setHours(Math.floor(mins/60), mins%60,0,0); if(d<careDayStart) d.setDate(d.getDate()+1); slots.push({id:`${plan.id}-${d.toISOString()}`,scheduledAt:d.toISOString(),medicationCode:plan.id.includes('tramadol')?'tramadol':'paracetamol',doseMg:plan.tramadolMg||plan.paracetamolMg||0,planType:'regular',roundLabel:t,planId:plan.id});});}); return slots.sort((a,b)=>new Date(a.scheduledAt)-new Date(b.scheduledAt)); }
function matchActualEventsToPlannedSlots(events, plannedSlots){ const used=new Set(); const matched=[]; plannedSlots.forEach((slot)=>{ const sMs=new Date(slot.scheduledAt).getTime(); let best=null; (events||[]).forEach((e)=>{ if(e.status!=='taken'||e.planId!==slot.planId||used.has(e.id)) return; const t=new Date(e.actualAt).getTime(); if(t<sMs-90*60000||t>sMs+3*3600000) return; const d=Math.abs(t-sMs); if(!best||d<best.d) best={e,d}; }); if(best){used.add(best.e.id); matched.push({slot,event:best.e});} else matched.push({slot,event:null}); }); const extras=(events||[]).filter((e)=>e.status==='taken'&&!used.has(e.id)&&e.planId!=='prn'); const breakthroughs=(events||[]).filter((e)=>e.status==='taken'&&e.planId==='prn'); return {matched,extras,breakthroughs}; }
function getSlotStatus(slot, event, now){ const sMs=new Date(slot.scheduledAt).getTime(); const n=now.getTime(); if(event) return 'taken'; if(n<sMs) return 'upcoming'; if(n<=sMs+30*60000) return 'due'; if(n<=sMs+2*3600000) return 'late'; return 'missed'; }
function getDeviationMinutes(slot,event){ if(!event) return null; return Math.round((new Date(event.actualAt)-new Date(slot.scheduledAt))/60000); }
function getPainPoints(events,start,end){ return (events||[]).flatMap((e)=>[{value:e.painBefore,at:e.actualAt,label:'before',planId:e.planId},{value:e.painAfter,at:e.actualAt,label:'after',planId:e.planId}]).filter((p)=>Number.isFinite(Number(p.value))&&new Date(p.at)>=start&&new Date(p.at)<=end).sort((a,b)=>new Date(a.at)-new Date(b.at)); }
function getDoseImpact(events){ const painLogs=(events||[]).flatMap((e)=>[{at:new Date(e.actualAt),value:e.painBefore,planId:e.planId},{at:new Date(e.actualAt),value:e.painAfter,planId:e.planId}]).filter((x)=>Number.isFinite(Number(x.value))).sort((a,b)=>a.at-b.at); const groups={'regular-tramadol':[],'prn':[],'regular-paracetamol':[]}; (events||[]).forEach((e)=>{ if(!groups[e.planId]) return; const at=new Date(e.actualAt); let before=Number.isFinite(Number(e.painBefore))?Number(e.painBefore):null; if(before===null){ const prev=[...painLogs].reverse().find((p)=>p.at<=at); if(prev) before=Number(prev.value);} const after=painLogs.find((p)=>p.at>at&&p.at-at>=3600000&&p.at-at<=4*3600000); if(before===null||!after) return; groups[e.planId].push({before,after:Number(after.value)}); }); return groups; }
function getBreakthroughStats(events, rangeStart, rangeEnd){ const rows=[]; for(let d=new Date(rangeStart); d<=rangeEnd; d.setDate(d.getDate()+1)){ const key=new Date(d); key.setHours(0,0,0,0); const next=new Date(key); next.setDate(next.getDate()+1); const count=(events||[]).filter((e)=>e.planId==='prn'&&e.status==='taken'&&new Date(e.actualAt)>=key&&new Date(e.actualAt)<next).length; rows.push({date:new Date(key),count}); } const prn=(events||[]).filter((e)=>e.planId==='prn'&&e.status==='taken'&&new Date(e.actualAt)>=rangeStart&&new Date(e.actualAt)<=rangeEnd); const avgBefore=prn.map((e)=>e.painBefore).filter((v)=>Number.isFinite(Number(v))); const windowCounts={morning:0,afternoon:0,evening:0,night:0}; prn.forEach((e)=>{ const h=new Date(e.actualAt).getHours(); if(h<6) windowCounts.night++; else if(h<12) windowCounts.morning++; else if(h<18) windowCounts.afternoon++; else windowCounts.evening++;}); const top=Object.entries(windowCounts).sort((a,b)=>b[1]-a[1])[0]; return {rows,avgBefore:avgBefore.length?avgBefore.reduce((a,b)=>a+Number(b),0)/avgBefore.length:null,topWindow:top&&top[1]>0?top[0]:null}; }
function getTimingWarnings(events){ const taken=(events||[]).filter((e)=>e.status==='taken').sort((a,b)=>new Date(a.actualAt)-new Date(b.actualAt)); const byPlan={}; taken.forEach((e)=>{(byPlan[e.planId]=byPlan[e.planId]||[]).push(e);}); const calc=(id)=>{const arr=byPlan[id]||[]; let min=null; for(let i=1;i<arr.length;i++){ const gap=(new Date(arr[i].actualAt)-new Date(arr[i-1].actualAt))/60000; if(min===null||gap<min) min=gap;} return min;}; return {paracetamol:calc('regular-paracetamol'),tramadol:calc('regular-tramadol')}; }
function HistoryPanel({ events, plans, filter, onFilterChange, effectiveNow, onDelete }) { const careDayStart=getCareDayStart(effectiveNow); const careDayEnd=getCareDayEnd(effectiveNow); const planned=generatePlannedSlots(plans,careDayStart); const {matched,breakthroughs}=matchActualEventsToPlannedSlots(events,planned); const takenCount=matched.filter((m)=>m.event).length; const painToday=getPainPoints(events,careDayStart,careDayEnd); const impact=getDoseImpact(events); const btStats=getBreakthroughStats(events,getHistoryFilterStart(filter,effectiveNow)||careDayStart,effectiveNow); const warnings=getTimingWarnings(events); const grouped=groupEventsByDate(filterEventsByHistory(events,filter,effectiveNow)); return <div className="space-y-4"><section className="insight-card"><h2 className="text-lg font-semibold">Today summary</h2><p className="text-sm text-slate-600 mt-1">{takenCount} of {planned.length} meds logged</p><p className="text-sm text-slate-600">{painToday.length?`Pain logs today: ${painToday.length}`:'Pain data missing today.'}</p></section><section className="insight-card"><h2 className="text-lg font-semibold mb-2">Medication timeline</h2><div className="overflow-x-auto"><div className="min-w-[680px] text-xs font-mono text-slate-500 flex justify-between mb-2"><span>6 AM</span><span>12 PM</span><span>6 PM</span><span>12 AM</span></div><div className="space-y-2">{matched.map((m)=><div key={m.slot.id} className="text-sm">{m.slot.roundLabel} {m.slot.medicationCode[0].toUpperCase()} · {getSlotStatus(m.slot,m.event,effectiveNow)} {m.event?`${getDeviationMinutes(m.slot,m.event)>0?'+':''}${getDeviationMinutes(m.slot,m.event)}m`:''}</div>)}</div></div></section><section className="insight-card"><h2 className="text-lg font-semibold">Timing drift</h2>{matched.map((m)=>{const dev=getDeviationMinutes(m.slot,m.event);const st=getSlotStatus(m.slot,m.event,effectiveNow);const w=dev==null?0:Math.min(Math.abs(dev),120)/120*100; return <div key={m.slot.id} className="mt-2 text-sm"><div>{m.slot.roundLabel} {m.slot.medicationCode[0].toUpperCase()} {dev==null?st:`${dev>0?'+':''}${dev}m`}</div><div className="h-1.5 bg-slate-100 rounded"><div className="h-1.5 bg-slate-400 rounded" style={{width:`${w}%`}}/></div></div>;})}</section><section className="insight-card"><h2 className="text-lg font-semibold">Pain trend</h2><div className="text-sm text-slate-600">{painToday.length?painToday.map((p)=>`${formatTime(p.at)} ${p.value}`).join(' · '):'No pain scores logged for this range.'}</div></section><section className="insight-card"><h2 className="text-lg font-semibold">Pain change after dose</h2>{Object.entries(impact).map(([k,v])=><div key={k} className="text-sm mt-2">{k}: {v.length?`${(v.reduce((a,b)=>a+b.before,0)/v.length).toFixed(1)} → ${(v.reduce((a,b)=>a+b.after,0)/v.length).toFixed(1)} (n=${v.length})`:'Not enough paired data'}</div>)}</section><section className="insight-card"><h2 className="text-lg font-semibold">Breakthrough use</h2>{btStats.rows.map((r)=><div key={r.date.toISOString()} className="text-sm font-mono">{formatDate(r.date)} {'█'.repeat(r.count)||'—'} {r.count}</div>)}<div className="text-sm text-slate-600 mt-2">{btStats.avgBefore!=null?`Avg pain before breakthrough: ${btStats.avgBefore.toFixed(1)} / 10`:'No breakthrough doses logged in this range.'}</div></section><section className="insight-card"><h2 className="text-lg font-semibold">Timing checks</h2><p className="text-sm text-slate-600">Paracetamol shortest gap: {warnings.paracetamol?`${Math.floor(warnings.paracetamol/60)}h${String(Math.round(warnings.paracetamol%60)).padStart(2,'0')}`:'—'}</p><p className="text-sm text-slate-600">Tramadol shortest gap: {warnings.tramadol?`${Math.floor(warnings.tramadol/60)}h${String(Math.round(warnings.tramadol%60)).padStart(2,'0')}`:'—'}</p></section><details className="insight-card"><summary className="cursor-pointer font-semibold">Log</summary><div className="mt-3 space-y-2">{Object.keys(grouped).sort((a,b)=>new Date(b)-new Date(a)).map((key)=><div key={key}><div className="text-xs text-slate-500">{formatDate(key)}</div>{grouped[key].map((e)=><div key={e.id} className="text-sm flex justify-between"><span>{formatTime(e.actualAt)} {e.planId==='prn'?'▲':e.planId.includes('tramadol')?'T':'P'} {formatStatus(e.status)}</span><button onClick={()=>onDelete(e.id)} className="text-slate-400">×</button></div>)}</div>)}</div></details></div>; }
