...
function LoginScreen({ mode, onModeChange, credentials, onCredentialsChange, onSubmit, submitting, error }) {
  return <div className="min-h-screen bg-slate-50 text-slate-900"><main className="mx-auto flex min-h-screen max-w-md items-center px-4 py-10"><section className="w-full rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><h1 className="text-2xl font-semibold text-slate-900">Medication Schedule</h1><p className="mt-2 text-sm text-slate-600">Sign in to access the tracker.</p><div className="mt-5 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1"><button onClick={() => onModeChange("signin")} className={classNames("rounded-xl px-3 py-2 text-sm font-semibold transition", mode === "signin" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600")}>Sign in</button><button onClick={() => onModeChange("signup")} className={classNames("rounded-xl px-3 py-2 text-sm font-semibold transition", mode === "signup" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600")}>Create account</button></div><form className="mt-5 space-y-4" onSubmit={onSubmit}><Field label="Email"><input type="email" autoComplete="email" required value={credentials.email} onChange={(event) => onCredentialsChange({ ...credentials, email: event.target.value })} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-slate-300" /></Field><Field label="Password"><input type="password" autoComplete={mode === "signin" ? "current-password" : "new-password"} required minLength={6} value={credentials.password} onChange={(event) => onCredentialsChange({ ...credentials, password: event.target.value })} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-slate-300" /></Field>{error ? <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{error}</div> : null}<button type="submit" disabled={submitting} className="w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white disabled:opacity-60">{submitting ? "Please wait..." : mode === "signin" ? "Sign in" : "Create account"}</button></form></section></main></div>;
}
...
function MedicationSchedulePWA() {
  ...
  useEffect(() => { if (!supabase) { setAuthLoading(false); return; } let mountedAuth = true; (async () => { const { data, error } = await supabase.auth.getSession(); if (error) logSupabaseError("auth:getSession", error); if (mountedAuth) { setSession(data?.session || null); setAuthLoading(false); } })(); const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => { setSession(nextSession || null); setAuthLoading(false); setAuthError(""); }); return () => { mountedAuth = false; listener?.subscription?.unsubscribe(); }; }, []);
  ...
  async function handleAuthSubmit(event) { event.preventDefault(); if (!supabase) return; setAuthSubmitting(true); setAuthError(""); try { if (authMode === "signin") { const { error } = await supabase.auth.signInWithPassword({ email: authForm.email.trim(), password: authForm.password }); if (error) throw error; } else { const { error } = await supabase.auth.signUp({ email: authForm.email.trim(), password: authForm.password }); if (error) throw error; } } catch (error) { setAuthError(error?.message || "Authentication failed."); } finally { setAuthSubmitting(false); } }
  async function handleSignOut() { if (!supabase) return; const { error } = await supabase.auth.signOut(); if (error) logSupabaseError("auth:signOut", error); }
