...
const classNames = (...parts) => clsx(parts);
const formatTime = (d) => format(new Date(d), "p");
const formatDateTime = (d) => format(new Date(d), "EEE, MMM d, p");
const formatRelative = (ms) => formatDistanceStrict(new Date(0), new Date(Math.abs(ms)), { unit: "minute" });
...
function OverviewCard({onOpen, events, now}){const last=events.at(-1);const nextRegularAt = last ? addMinutes(new Date(last.actualAt), 360) : null;return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200"><ClockIcon className="h-3.5 w-3.5" />Updated {formatTime(now)}</div><h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{last?`Next regular around ${formatTime(nextRegularAt)}`:"No regular dose logged yet"}</h2><p className="mt-1 text-lg text-slate-700">{last?`Last event ${formatDateTime(last.actualAt)}`:"Log the first regular dose to start the rolling schedule."}</p><div className="mt-5 grid gap-3 sm:grid-cols-3"><button onClick={()=>onOpen("regular","taken")} className="rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white">Log regular dose</button><button onClick={()=>onOpen("prn","taken")} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Log as-needed dose</button><button onClick={()=>onOpen("prn","not-needed")} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">Mark PRN not needed</button></div></section>}
...
function MedicationSchedulePWA(){
  ...
  const addEvent=(payload)=>{setData((c)=>({...c,events:[...c.events,{id:nanoid(), ...payload}]})); closeSheet();};
