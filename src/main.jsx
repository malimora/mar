...
function OverviewCard({ regularState, prnState, lastRegular, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  ...
  const primaryTitle = regularState.status === "empty" ? "No regular dose logged yet" : regularState.status === "overdue" ? "Regular dose overdue" : `Next regular at ${formatTime(regularState.nextDueAt)}`;
  ...
  const prnLine = prnState.available ? "As-needed Tramadol is also available now if pain requires it." : `As-needed Tramadol becomes available in ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}.`;
