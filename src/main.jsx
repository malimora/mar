...
function MedicationSchedulePWA() {
  ...
  async function saveSettings(nextSettings) { updateSettings(nextSettings); if (!mounted || !supabase || !authReady || !session?.user?.id || !hasLoadedRemoteState) return; setSettingsSaveStatus("saving"); try { const planRows = nextSettings.plans.map((plan) => mapPlanToUserPlanRow(plan)).filter(Boolean); if (planRows.length) { const { error: plansError } = await supabase.from("user_plans").upsert(planRows, { onConflict: "plan_id" }); if (plansError) throw plansError; } const payload = { id: 1, tramadol_spacing_minutes: nextSettings.tramadolSpacingMinutes, reminders: nextSettings.reminders }; const { error: settingsError } = await supabase.from("user_settings").upsert(payload, { onConflict: "id" }); if (settingsError) throw settingsError; logSupabase("settings:sync:success", { planCount: planRows.length }); setSettingsSaveStatus("saved"); if (typeof window !== "undefined") { window.setTimeout(() => setSettingsSaveStatus("idle"), 2000); } } catch (error) { logSupabaseError("settings:sync", error); setSettingsSaveStatus("error"); } }
