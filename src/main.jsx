...
function MedicationSchedulePWA() {
  ...
  async function addEvent(payload) { const plan = planMap[payload.planId]; if (!plan) return; const nextEvent = { id: crypto.randomUUID(), planId: payload.planId, status: payload.status, actualAt: new Date(payload.actualAt).toISOString(), note: payload.note || "", painBefore: payload.painBefore, painAfter: payload.painAfter, createdAt: new Date().toISOString(), medications: payload.status==="taken"?(payload.medications||[]):[], containsTramadol: (payload.medications||[]).some((m)=>m.code==="tramadol") }; setData((current) => ({ ...current, events: normalizeEvents([...current.events, nextEvent], current.settings.plans) })); if (supabase) { try { logSupabase("event:insert:attempt", { eventId: nextEvent.id, planId: nextEvent.planId, status: nextEvent.status, actualAt: nextEvent.actualAt }); const payloadItems = nextEvent.medications.map((m)=>({ medication_code: m.code, dose_mg: Number(m.doseMg) })); const { data: insertedId, error } = await supabase.rpc("log_dose_event", { p_plan_id: nextEvent.planId, p_status: nextEvent.status, p_actual_at: nextEvent.actualAt, p_note: nextEvent.note || "", p_pain_before: nextEvent.painBefore, p_pain_after: nextEvent.painAfter, p_items: payloadItems }); if (error) throw error; if (insertedId) { nextEvent.id = insertedId; } logSupabase("event:insert:success", { eventId: nextEvent.id }); } catch (error) { logSupabaseError("event:insert", error, { eventId: nextEvent.id }); } } else { logSupabase("event:insert:skipped", { reason: "missing supabase client", hasSupabaseClient: Boolean(supabase), eventId: nextEvent.id }); } closeSheet(); }
