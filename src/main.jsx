...
function OverviewCard({ focusRegularPlan, focusRegularState, prnState, lastTramadol, paracetamolToday, settings, effectiveNow, onOpen }) {
  const tone = focusRegularState?.status === "overdue" ? "border-rose-200 bg-rose-50/80" : "border-slate-200 bg-white";
  ...
  const overdueDuration = focusRegularState?.nextDueAt ? formatRelative(effectiveNow.getTime() - focusRegularState.nextDueAt.getTime()) : null;
  const planLabel = focusRegularPlan?.label || "Regular dose";
  const primaryTitle = !focusRegularState ? "No regular plans configured" : focusRegularState.status === "empty" ? `${planLabel} not logged yet` : focusRegularState.status === "overdue" ? `${planLabel} overdue` : `${planLabel} due ${formatTime(focusRegularState.nextDueAt)}`;
  const primarySubtitle = !focusRegularState ? "Add at least one required plan in Settings." : focusRegularState.status === "empty" ? "Log the first event to start the rolling schedule." : focusRegularState.status === "overdue" ? `${overdueDuration} late` : `In ${formatRelative(focusRegularState.nextDueAt.getTime() - effectiveNow.getTime())}`;
  const dueLine = focusRegularState?.status === "overdue" && focusRegularState.nextDueAt ? `Due since ${formatDateTime(focusRegularState.nextDueAt)}.` : null;
  ...
  const requiredPlans = (settings.plans || []).filter((plan) => plan.kind === "required");
  return <section className={classNames("rounded-3xl border p-5 shadow-sm sm:p-6", tone)}><div className="flex items-start justify-between gap-3"><h2 className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">{primaryTitle}</h2><div className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-slate-600 ring-1 ring-slate-200"><ClockIcon className="h-3.5 w-3.5" />Updated {formatTime(effectiveNow)}</div></div>{focusRegularPlan?.medication?<p className="mt-1 text-sm text-slate-600">Medication: {focusRegularPlan.medication}</p>:null}<p className="mt-2 text-xl font-semibold text-slate-900">{primarySubtitle}</p>{dueLine?<p className="mt-1 text-base text-slate-700">{dueLine}</p>:null}<div className="mt-3 space-y-1 text-sm leading-6 text-slate-700"><div>{prnLine}</div><div>{safetyLine}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:max-w-xl">{requiredPlans.map((plan)=><button key={plan.id} onClick={() => onOpen(plan.id, "taken")} className="min-h-12 rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800">Log {plan.label.toLowerCase()}</button>)}<button onClick={() => onOpen("prn", "taken")} className="min-h-12 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Log breakthrough dose</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><OverviewMetric label="Focus plan" value={focusRegularPlan?.label || "—"} /><OverviewMetric label="Last tramadol" value={lastTramadol ? formatDateTime(lastTramadol.actualAt) : "Not logged"} /><OverviewMetric label="Breakthrough dose" value={prnState.available ? "Available now" : `In ${formatRelative(prnState.availableAt.getTime() - effectiveNow.getTime())}`} /><OverviewMetric label="Paracetamol since midnight" value={`${paracetamolToday} mg`} /></div></section>;
}
...
function MedicationSchedulePWA() {
  ...
  const effectiveNow = liveNow; const planMap = useMemo(() => getPlanMap(data.settings.plans), [data.settings.plans]); const regularPlans = useMemo(() => data.settings.plans.filter((plan) => plan.kind === "required"), [data.settings.plans]); const regularStates = useMemo(() => regularPlans.map((plan) => ({ plan, state: getRegularPlanState(plan, data.events, effectiveNow) })), [regularPlans, data.events, effectiveNow]); const focusRegular = useMemo(() => { if (!regularStates.length) return null; const rank = (item) => { if (item.state.status === "overdue") return 0; if (item.state.status === "due-soon") return 1; if (item.state.status === "tracking") return 2; if (item.state.status === "upcoming") return 3; return 4; }; return [...regularStates].sort((a,b)=>{ const r=rank(a)-rank(b); if (r!==0) return r; const aMs=a.state.nextDueAt?new Date(a.state.nextDueAt).getTime():Number.POSITIVE_INFINITY; const bMs=b.state.nextDueAt?new Date(b.state.nextDueAt).getTime():Number.POSITIVE_INFINITY; return aMs-bMs; })[0]; }, [regularStates]); const prnPlan = planMap.prn; const prnState = useMemo(() => getPrnPlanState(prnPlan, data.events, data.settings, effectiveNow), [prnPlan, data.events, data.settings, effectiveNow]); const lastTramadol = useMemo(() => getLatestTramadolEvent(data.events, effectiveNow), [data.events, effectiveNow]); const paracetamolToday = useMemo(() => getParacetamolToday(data.events, data.settings.plans, effectiveNow), [data.events, data.settings.plans, effectiveNow]);
